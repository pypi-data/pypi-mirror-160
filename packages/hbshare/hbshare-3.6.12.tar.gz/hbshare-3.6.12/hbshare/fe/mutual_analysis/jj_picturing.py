...
def get_pic_from_localdb(jjdm,if_percentage=True,show_num=None):
    ...
    latest_date = pd.read_sql(
        "select max(asofdate) as asofdate from hbs_ticker_contribution",
        con=localdb)['asofdate'][0]
    ...
    latest_date=pd.read_sql(
        "select max(asofdate) as asofdate from nav_jj_ret_analysis",con=localdb)['asofdate'][0]
    sql="SELECT * from nav_jj_ret_analysis where {0} and asofdate='{1}' "\
        .format(jjdm,latest_date)
    jj_performance=pd.read_sql(sql,con=localdb).fillna(0)

    latest_date=pd.read_sql(
        "select max(asofdate) as asofdate from nav_hurst_index",con=localdb)['asofdate'][0]
...
def industry_pic_all(jj_base_info,th1=0.5, th2=0.5,if_prv=False,fre='Q'):
...
def style_pic_all(jj_base_info,fre, th1=0.5, th2=0.5, if_percentage=True,if_prv=False,fre2='Q'):
...
def save_entire_jjpic2db(if_prv=False,fre='Q'):
    if(if_prv):
    ...
    ind_cen=industry_pic_all(jj_base_info,if_prv=if_prv,fre=fre)
    # style_pic_all(jj_base_info,fre='M', if_percentage=False,if_prv=if_prv,fre2=fre)
...
if __name__ == '__main__':
    ...
    # jj_performance,industry_contribution_list,ticker_con,theme_exp,ind2_exp\
    #     ,industry_contribution_perweight_list,style_exp,size_exp= \
    #     get_pic_from_localdb("jjdm ='166006'")
    #
    # pic_df=pd.merge(industry_p[['基金简称', '一级行业类型', '一级行业集中度', '一级行业换手率', '前五大行业', '一级行业策略','二级行业策略','行业精选个股策略', '龙头占比(时序均值)','二级行业类型', '二级行业集中度', '二级行业换手率','三级行业类型', '三级行业集中度',
    #    '三级行业换手率', '二级行业前20大']]
    #                 ,value_p_hbs[['基金简称','风格类型','风格偏好','成长绝对暴露(持仓)',
    #    '价值绝对暴露(持仓)', '集中度排名(持仓)', '换手率排名(持仓)']],how='left',on='基金简称')
    ...
    #save_entire_jjpic2db(if_prv=True,fre='Q')
    ...
    jjdm_list=['001856','000924','001410','001476','001975','005968','270028','377240']
