...
from hbshare.fe.factor_analysis import simple_factor_generator as sfg
...
def pic_grouping(df,pic_title,fre,pool_size,width=1000,height=1000):
    ...
    ext_ret=np.power(np.max([draw_df.iloc[-1]['group_4'],draw_df.iloc[-1]['group_0']]), 1 / mi)-\
    np.power( draw_df.iloc[-1]['benchmark_ret'],1 / mi)
    print('annually ext ret for best group is {}%'.format(ext_ret*100))

    pool_ext_ret=np.power(np.max([draw_df.iloc[-1]['group_top_{}'.format(pool_size)],draw_df.iloc[-1]['group_last_{}'.format(pool_size)]]), 1 / mi)-\
    np.power( draw_df.iloc[-1]['benchmark_ret'],1 / mi)
    print('annually ext ret for mimic pool is {}%'.format(pool_ext_ret*100))
...
def grouping(groupdf,retdf,factor_name,col_list,factor_date,pool_size=30,ret_date=None):
...
def single_factor_ic(factor_df,factor_name,fre,pool_size,fund_flag=True):
...
if __name__ == '__main__':
    ...
    #                    .format(factor_name)
    #                    ,con=localdb)
    # groupdf,factrodf=single_factor_ic(raw_df,factor_name=factor_name,fre='Q',fund_flag=True)
    ...
    # groupdf,factrodf = single_factor_ic(raw_df, factor_name=factor_name, fre='M', fund_flag=True)
    # pic_grouping(groupdf, '超额月度',fre='M')
