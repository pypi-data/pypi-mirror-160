...
class TriggerJob(Job):
    ...
    def __init__(self, opts: TriggerJobOpts) -> None:

        if opts.includes and opts.project:
            raise ValueError(("You cannot specify 'include' and 'project' together. Either 'include' or 'project' is possible."))
        if not opts.includes and not opts.project:
            raise ValueError("Neither 'includes' nor 'project' is given.")

        super().__init__(name=opts.name, stage=opts.stage, script="none")

        self._project = opts.project
        self._branch = opts.branch
        self._strategy = opts.strategy

        if not opts.includes:
            self._includes = None
        elif isinstance(opts.includes, Include):
            self._includes = [opts.includes]
        elif isinstance(opts.includes, list):
            if len(opts.includes) > 3:
                raise ValueError(
