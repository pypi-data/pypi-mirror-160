...
class TestAutoAIRemote(AbstractTestTSAsync, unittest.TestCase):
    ...
    def test_09a_predict_exogenous_pipelines_as_sklearn(self):
        ...
        TestAutoAIRemote.summary_exogenous = self.winning_pipelines_summary[pipelines_summary['Type'] == 'exogenous']
        TestAutoAIRemote.summary_non_exogenous = self.winning_pipelines_summary[
            pipelines_summary['Type'] == 'non-exogenous']
    ...
    def test_09c_predict_non_exogenous_pipelines_as_lale(self):
        ...
        for pipeline_name in self.summary_non_exogenous.index:
            ...
            try:
                pipeline_model = self.remote_auto_pipelines.get_pipeline(pipeline_name, astype='lale')
                predictions = pipeline_model.predict(self.scoring_df)
                print(predictions)
                self.assertGreater(len(predictions), 0)
            except Exception as e:
                failed_pipelines.append(pipeline_name)
