...
class LatinHyperCubeSampling(SampleGenerator):
    ...
    continuous and categorical variables. Samples genereated will be limited by the variables lower/upper bounds,
            ...
            if constraints is not None:
                ...
                while X.shape[0] < self.num_new_points:
                    if X.shape[0] == self.num_new_points - 1:
                        # The last set of variables can some times not be selected when it stays on LHS-type selection
                        # and gets stuck in the while loop. Therefore, the last set of variables is selected randomly
...
class RandomSampling(SampleGenerator):
    ...
    def generate_samples(
        ...
        if variables.num_cat_var > 0:
            ...
            if constraints is not None:
                X = X[constraints.bool_evaluate_constraints(X=X, descriptor_to_name=False)]
                while X.shape[0] < self.num_new_points:
                    remainder = sampler(self.num_new_points - X.shape[0])
                    X = np.vstack((X, remainder))
                    X = X[constraints.bool_evaluate_constraints(X=X, descriptor_to_name=False)]
