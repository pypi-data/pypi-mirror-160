...
class HistogramMatcher(TransformationBasic):
    ...
    _guipi_dictionary = {'reference_slice': GuiPI(int)}
    ...
    def _match_histogram(self,x):
        ...
        for i in range(1,self.reference_slice):
            ...
            self.progress_bar(len(hm_x)-1,len(x)-1,15,'{}/{}'.format(len(hm_x)-1,len(x)))

        hm_x = list(reversed(hm_x))
        self.progress_bar(len(hm_x),len(x)-1,15,'{}/{}'.format(len(hm_x),len(x)))
        for i in range(self.reference_slice,len(x)):
