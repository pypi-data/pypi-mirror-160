...
class Bar_thesis_Augment(Augment):
    ...
    def augment_function_1(self, c, beta, agumented_data=None):
    ...
    def augment_function_2(self, c, beta):
        Y = self.last_samples_uncensord_only_microbiom.sub(c, axis=1)
        Y = np.sqrt(np.square(Y).sum(axis=1))
        di = 1 / Y
        return di

    def augment_function_3(self, c, beta):
        Y = self.last_samples_uncensord_only_microbiom.sub(c, axis=1)
        Y = np.sqrt(np.square(Y).sum(axis=1))
        di = 1 / (1 + Y ** 2)
        return di
