...
class StrUtils:
    ...
    @classmethod
    def to_camel_dict(cls, dict_: Dict) -> Dict:

        return {cls.to_camel(key): val for key, val in dict_.items()}

    @classmethod
    def to_snake_dict(cls, dict_: Dict) -> Dict:

        return {cls.to_snake(key): val for key, val in dict_.items()}

    @classmethod
    def to_upper_camel_dict(cls, dict_: Dict) -> Dict:

        return {cls.to_upper_camel(key): val for key, val in dict_.items()}
