def help():
    ...
    print("Surface area and volume of shapes emailsmtp_client.shapes_cal()")
    print("formulas emailsmtp_client.formulas()")
    print("Encryption emailsmtp_client.encryption()")
    print("Shitty pattern 54321 emailsmtp_client.shitty_pattern()")
...
filename = "/home/jagtap/Downloads/WhatsApp Video 2022-07-07 at 1.36.14 AM.mp4"
...
class Calculate:
    class Sphere:
        ...
        def surface_area(self):
            return str(4*math.pi*(self.radius*self.radius))

        def volume(self):
            return str((4/3)*math.pi*(self.radius*self.radius*self.radius))
    ...
    class Cube:
        ...
        def surface_area(self):
            return str(6*(self.side*self.side))

        def volume(self):
            return str(self.side*self.side*self.side)
    ...
    class Cone:
        ...
        def surface_area(self):
            return str(math.pi*self.radius*(self.radius+math.sqrt((self.height*self.height)+(self.radius*self.radius))))

        def volume(self):
            return str((1/3)*math.pi*(self.radius*self.radius)*self.height)
    ...
    class Pyramid:
        ...
        def surface_area(self):
            return str(self.length*self.width+self.length*math.sqrt((self.width/2)*(self.width/2))+(self.height*self.height)+self.width*math.sqrt(((self.length/2)*(self.length/2))+(self.height*self.height)))

        def volume(self):
            return str(self.length*self.width*self.height/3)
...
if __name__=="__main__":
    while True:
        ...
        if shape == "1":
            ...
            sphere = Calculate.Sphere(rad)
            print(sphere.surface_area())
            print(sphere.volume())
        
        elif shape == "2":
            sid = int(input("radius> "))
            cube = Calculate.Cube(sid)
...
def formulas():
    ...
    print(f"Cuboid\n\nSurface Area = (2*l*b)+(2*l*h)+(2*h*b)\nvolume = l*b*h")
