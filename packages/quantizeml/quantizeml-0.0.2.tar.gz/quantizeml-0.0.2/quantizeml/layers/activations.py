...
def act_to_qlayer(act, config):
    """Get quantized activation layer from string.

    This is needed because one cannot serialize a class in layer.get_config, the string is thus
    serialized instead.

    Args:
        act (str): string that values in ['GeLU', 'ReLUx] and that allows to
            choose activation inside MLP.
        config (str): quantization configuration

    Returns:
        tf.keras.layers: the activation layer class
    """
    if act == "GeLU":
        act_funct = QuantizedGELU(quant_config=config)
    elif act[:4] == 'ReLU':
        try:
            max_value = int(act[4:])
        except ValueError:
            raise ValueError(
                "ReLU must be in the form 'ReLUx', where x is the max-value")
        act_funct = QuantizedReLU(quant_config=config, max_value=max_value)
    else:
        raise NotImplementedError(
            f"act should be in ['GeLU', 'ReLUx'] but received {act}.")

    return act_funct


@tf.keras.utils.register_keras_serializable()
class QuantizedGELU(keras.layers.Layer):
    ...
    Args:
        quant_config (dict): quantization configuration of input/ouput quantizers.
    """
    unsupported_args = {
        'approximate': True}
