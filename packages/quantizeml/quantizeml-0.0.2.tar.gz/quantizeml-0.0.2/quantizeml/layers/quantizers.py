...
class Quantizer(Layer):
    ...
    def _get_training_value(self, training=None):
        """Read training mode from the context. Function taken of
        `BatchNormalizationLayer <https://bit.ly/3xNYfxg>`_ code.

        Args:
            training (bool, optional): the training mode. Defaults to None.

        Returns:
            bool: the training mode: True if training, False if inference.
        """
        if training is None:
        ...
        if self._USE_V2_BEHAVIOR:
            ...
            if not self.trainable:
                ...
                training = False
        return control_flow_util.constant_value(training)
    ...
    def call(self, inputs, training=None):
        """Update the weights from moving average, and quantize the inputs in three steps:
            1. Get information of training mode
            2. Update the weights from moving average
        ...
        if training:  # Retrieve information from the inputs and update the weights
...
class UniformQuantizer(Quantizer):
    ...
    def __init__(self, bitwidth, signed=True, scale_bits=24, **kwargs):
