...
from dataproc.crawlers.parsers.mppage import page_ml3
...
class BatchPageActor(LocalActorModel):
    ...
    def execute(self, model, msg: bytes):
        ...
        table = asyncio.run(self.crawler.fetch2(urls))
        dft = table.to_pandas()
        pages = []
        for x in dft.index:
...
async def batch_crawl_pages_async(urls, conf: CrawlerConfig):
    bulks = list(partition_all(conf.chunk_size, urls))
    tasks = list(partition_all(conf.workers, bulks))
    broker = Broker()
    broker.mp_start()
    bpa = BatchPageActor(broker_back=broker.be,
                         broker_front=broker.fe, num_cpus=conf.workers)
    proc = bpa.mp_start(config=conf)

    pages = []
    for ix, chunks in enumerate(tasks):
        futures = [bpa.submit(json.dumps(c).encode()) for c in chunks]
        results_chunk = await asyncio.gather(*futures)
        # results_chunk = loop.run_until_complete(_submit_wrapper(pa, chunks))
