...
class SideBySide:
    ...
    def run(self, test_name="", new_test_name="", platform="", new_platform="", vismetPath="", metric="speedindex", base_branch="autoland", new_branch="autoland", base_revision="", new_revision="", cold="", warm="", most_similar="", original=False, search_crons=False, overwrite=True, skip_download=False, skip_slow_gif=False):
        ...
        if metric != "similarity":
            ...
            org_base_vismet = open_and_organize_perfherder(base_vismet, metric)
            org_new_vismet = open_and_organize_perfherder(new_vismet, metric)
        ...
        if run_cold:
            ...
            gif_output_name = self.convert_mp4_to_gif(output_name, gif_output_name)
            print(
            ...
            if not skip_slow_gif:
                gif_output_name = self.convert_mp4_to_gif(
        ...
        if run_warm:
            ...
            gif_output_name = self.self.convert_mp4_to_gif(output_name, gif_output_name)
            print(
