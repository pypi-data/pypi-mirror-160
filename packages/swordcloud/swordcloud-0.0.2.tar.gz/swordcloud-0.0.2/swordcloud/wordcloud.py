...
from processing import plot_TSNE, embed_w2v, generate_kmeans_frequencies, kmeans_cluster, rank_kmeans
...
class WordCloud(object):
    ...
    def generate_from_text(self, text, tsne_plot=None, kmeans=False):
        ...
        if kmeans==True:
            ...
            kmeans_freq = generate_kmeans_frequencies(model, words, NUM_CLUSTERS=6, size_min=10,size_max=12)
            clus = kmeans_cluster(model)
            rank = rank_kmeans(kmeans_freq, rank_type='big')
            
        ###
        self.generate_from_frequencies(words, tsne_plot)
        return self

    def generate(self, text, tsne_plot=None, kmeans=False):
        """Generate wordcloud from text.
        ...
        self
        """
        return self.generate_from_text(text, tsne_plot, kmeans)
