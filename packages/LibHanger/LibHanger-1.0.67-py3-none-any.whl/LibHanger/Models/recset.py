...
from TestClass.log_race_id import log_race_id
from TestClass.trn_race_id import trn_race_id
from TestClass.trn_race_result import trn_race_result
...
class recset(Generic[T]):
    ...
    def __initdfrows(self, columns:list = None):
        ...
        # 列リスト生成
        colList = []
        for col in self.__columns if columns == None else columns:
            colList.append(col[0]) if columns == None else colList.append(col.key)
        ...
        # 主キーセット
        if len(self.__primaryKeys) > 0 and columns == None:
    ...
    def __initRows(self, columns:list = None):
        ...
        self.__rows = []
        self.__dfrows = self.__initdfrows(columns)
    ...
    def joinFilter(self, j:saJoin, isouter, w:saWhere, s:saOrderBy):
        ...
        for row in q:
            ...
            # DataFrame - add
            self.__addRecrowDataFrame(mainRow, rowIndex)
            # rowIndex++
            rowIndex += 1
