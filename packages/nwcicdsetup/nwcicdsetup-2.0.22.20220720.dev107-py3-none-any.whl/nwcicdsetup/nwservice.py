...
class NWTask:
    ...
    async def to_yaml_async(self) -> Dict[str, Any]:
        def job(commands: List[Union[str, Dict[str, Any]]]) -> Dict[str, Any]:
            steps: List[Union[str, Dict[str, Any]]] = ["run: git config --global --add safe.directory '/mnt/ramdisk'", "checkout"]
            steps += commands
            steps.insert(1, {
                "changeset_info": {
        ...
        if self.is_deployment and self.is_feature_branch and await self.is_changed_async:
            return {self.name:{
                "docker": [{"image": "cimg/base:stable"}],
                "resource_class": "small",
                "steps": [self.command_to_step("empty_job_deploy", {
                        "job_name": self.name,
                        "reason": "Changes to the deployment on the feature branch were found."
                    })]
                }
            }
