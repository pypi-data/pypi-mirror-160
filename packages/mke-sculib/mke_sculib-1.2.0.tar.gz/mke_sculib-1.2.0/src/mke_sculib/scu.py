...
class scu():
    ...
    def wait_track_end(self, timeout=600, query_delay=1.):
        ...
        # key1 = "acu.tracking.act_pt_end_index_a"
        # key2 = "acu.tracking.act_pt_act_index_a"
        
        # def tester(v):
        #     req = self.status_Value(key2)
        #     return (int(v) - int(req)) <= 0

        # self.wait_for_status(key1, None, tester=tester, timeout=timeout, query_delay=query_delay)
        ...
        self.wait_for_status(key, value, tester=tester, timeout=timeout, query_delay=query_delay)
...
#        log('logger state ')
    ...
    def start(self, az_start=None, el_start=None, band_start=None, az_speed=3, el_speed=1):
