...
class PiBatch:
    ...
    def connect(self) -> None:
        """Connect to PI Batch server"""
        if not hasattr(self, 'server'):
            try:
                server = PISDK().Servers[self.server_name]
    ...
    def close(self) -> None:
        """Close PI Batch server connection"""
    ...
    def search(
        ...
        """
        self.connect()
        db = self.server.PiModuleDB
        start_time, end_time, batch_id, product, procedure, sub_batches = self.prep_search_criteria(
            start_time,
            end_time,
            batch_id,
            product,
        ...
        try:
            unit_batches_raw = [
                PIUnitBatch(batch) for batch in db.PIUnitBatchSearch(
                    start_time,
                    end_time,
                    unit_id,
                    batch_id,
        ...
        sub_batches_raw = {
            unit_batch.UniqueID: unit_batch.PISubBatches
    ...
    def parse_unit_batches(self, unit_batches_raw: list, now: str) -> pd.DataFrame:
        self.connect()
    ...
    def parse_sub_batches(self, sub_batches_raw: dict, now: str) -> pd.DataFrame:
        self.connect()
        parent_ids = []
        names = []
        start_times = []
