...
class Pad(OnnxOpConverter):
    ...
    def _impl_v1(cls, inputs, attr, params):
        ...
        #*BEGIN*#
        ## fix issue, onnx Pad-1 op's value attr is float type, and default valus is 0.0
        ## data: 21-10-16
        pad_value = attr.get("value")
    ...
    def _impl_v2(cls, inputs, attr, params):
        ...
        #*BEGIN*#
        ## fix issue, onnx Pad-2 op's value attr is float type, and default valus is 0.0
        ## data: 21-10-16
        pad_value = attr.get("value")
        if pad_value is None:
            attr["value"] = 0.0
...
class Slice(OnnxOpConverter):
    ...
    ##*BEGIN*#
    ## Add slice op axes input support in to onnx, fix tvm load onnx slice op issue:
    ## what tvm not handle the case that axes don't like [0, 1, 2, ..., N].
    ## data: 21-11-18
    @classmethod
    def _common_np(cls, starts, ends, steps, axes, shape):
        new_axes = []
        new_starts = []
        new_ends = []
        new_steps = []
    ...
    def _impl_v10(cls, inputs, attr, params):
        ...
        if axes is not None and has_static_axes():
            ...
            try:
                if (max(axes_np) + 1) != len(axes_np):
                    begin_np, end_np, strides_np, axes_np = cls._common_np(
                        begin_np, end_np, strides_np, axes_np, ishape
                    )
            except KeyError:
                pass
    ##*END*
...
class ConstantOfShape(OnnxOpConverter):
    ...
    def _impl_v9(cls, inputs, attr, params):
        ...
        ## data: 21-12-23
        output = _op.full(value, shape=inputs[0], dtype=dtype)
        ##*END*
        return output
...
class Resize(OnnxOpConverter):
    ...
    def _impl_v10(cls, inputs, attr, params):
        ...
        #*BEGIN*#
        ## bugfix: resize-10 only suport floor nearest mode
        ## data: 21-10-30
        ...
        if ndims == 3:
            ...
            out = _op.image.resize1d(inputs[0], out_size, "NCW", method, "asymmetric", nearest_mode)
        elif ndims == 4:
            out_size = fold_constant(_op.strided_slice(size, [2], [4]))
            out = _op.image.resize2d(inputs[0], out_size, "NCHW", method, "asymmetric", nearest_mode)
        elif ndims == 5:
            out_size = fold_constant(_op.strided_slice(size, [2], [5]))
            out = _op.image.resize3d(inputs[0], out_size, "NCDHW", method, "asymmetric", nearest_mode)
        else:
...
class PriorBox(OnnxOpConverter):
    """Operator converter for PriorBox"""

    @classmethod
    def _impl_v1(cls, inputs, attr, params):
        if len(inputs) != 2:
            raise ValueError("SSD PriorBox expects 2 inputs.")

        featuremap = inputs[0]
        data = inputs[1]

        data_type = infer_type(data)
        data_shape = get_const_tuple(data_type.checked_type.shape)
        ndim = len(data_shape)
        if ndim != 4:
            raise ValueError("SSD PriorBox expects 3D image.")

        featuremap_type = infer_type(featuremap)
        featuremap_shape = get_const_tuple(featuremap_type.checked_type.shape)
        fdim = len(featuremap_shape)
        if fdim != 4:
            raise ValueError("SSD PriorBox expects 3D feature map.")

        max_sizes = attr["max_sizes"]
        min_sizes = attr["min_sizes"]
        if min_sizes is None:
            raise ValueError("SSD PriorBox expects at least 1 min size.")

        aspect_ratios = attr["aspect_ratios"] if "aspect_ratios" in attr else (1.0,)
        clip = attr["clip"] if "clip" in attr else 0
        flip = attr["flip"] if "flip" in attr else 1
        img_sizes = attr["img_sizes"] if "img_sizes" in attr else (0, 0)
        offset = attr["offset"] if "offset" in attr else (0.5, 0.5)
        if type(offset) is tuple:
            offsets = offset
        else:
            offsets = (offset, offset)
        steps = attr["steps"] if "steps" in attr else (0.0, 0.0)
        variances = attr["variances"] if "variances" in attr else (0.1, 0.1, 0.2, 0.2)

        return  _op.vision.ssd_priorbox(featuremap, data, min_sizes, max_sizes, img_sizes, aspect_ratios, 
                                        steps, offsets, variances, clip, flip)
...
def _get_convert_map(opset):
    return {
        ...
        # SSD support
        "PriorBox": PriorBox.get_converter(opset),
