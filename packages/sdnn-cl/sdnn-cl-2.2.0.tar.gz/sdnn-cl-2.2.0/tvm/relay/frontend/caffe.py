...
class OperatorConverter(object):
    ...
    def convert_softmax(self, op):
        ...
        # enable softmax convert now
        out = _op.nn.softmax(in_expr, **parmas)
        return out

        # disable softmax to math convert now, because xnnc code issue
        # using the LUT softmax, limited in 65535 data
        # optimize as onnx frontend to avoid softmax issue in xnnc
        #axis = op.softmax_param.axis
        #ndim = len(_infer_shape(in_expr))
        #if axis < 0:
        #    axis += ndim
