...
# ssd_priorbox
def wrap_compute_ssd_priorbox(topi_compute):
    """Wrap multibox_prior compute"""

    def _compute_ssd_priorbox(attrs, inputs, _):
        """Compute definition of ssd_priorbox"""
        min_sizes = get_float_tuple(attrs.min_sizes)
        max_sizes = get_float_tuple(attrs.max_sizes)
        img_sizes = get_const_tuple(attrs.img_sizes)
        ratios = get_float_tuple(attrs.aspect_ratios)
        steps = get_float_tuple(attrs.steps)
        offsets = get_float_tuple(attrs.offsets)
        variances = get_float_tuple(attrs.variances)
        ...
        clip = bool(get_const_int(attrs.clip))
        variances = get_float_tuple(attrs.variances)
        return [topi_compute(inputs[0], inputs[1], min_sizes, max_sizes, img_sizes, ratios, steps, offsets, variances, clip, flip)]

    return _compute_ssd_priorbox


@override_native_generic_func("ssd_priorbox_strategy")
def ssd_priorbox_strategy(attrs, inputs, out_type, target):
    """ssd_priorbox generic strategy"""
    strategy = _op.OpStrategy()
    strategy.add_implementation(
        wrap_compute_ssd_priorbox(topi.vision.ssd.ssd_priorbox),
        wrap_topi_schedule(topi.generic.schedule_ssd_priorbox),
        name="ssd_priorbox.generic",
    )
    return strategy
