...
def _plot_pwc_pulse(name):
    return rf"""
    Plot the pulse.

    >>> fig = plt.figure()
    >>> plot_controls(fig, {{"$\\alpha$": result.output["{name}"]}})
    >>> plt.show()
    ...
    .. plot::
       ...
       with open("./pulse/pwc/{name}.json", "r") as f:
           data = json.load(f)

       fig = plt.figure()
       plot_controls(fig, {{"$\\alpha$": data}})
       plt.tight_layout()
       plt.show()
"""
...
@update_docstring(_plot_pwc_pulse)
...
def sech_pulse_pwc(
    ...
    _plot_pwc_pulse(sech_displaced)
