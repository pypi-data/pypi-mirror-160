...
#        return hasattr(node, 'value')
    ...
    async def _process_event(self):
        ...
        while True:
            ...
            e = self._events.popleft()
            #
            # skip current event loop batch (process one frame)
            await asyncio.sleep(0)
        self._input_task = None

    @staticmethod
    def create_for(user_interface):
        ...
        async def on_active_node_changed():
            if Keyboard.is_editable(user_interface.active_node):
                if user_interface.active_node != self.input_target_node:
                    ...
                    return
            #
            # at the time of writing the focus flag can only be obtained while the window is visible
            if not self.visible:
                return
            if self.focused:
                return
