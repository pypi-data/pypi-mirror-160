...
class DingTalkNotifyPlugin(CorePluginMixin, notify.NotificationPlugin):
    ...
    def _post(self, group, project):
        ...
        project_obj = group.__dict__['_state'].__dict__['fields_cache']['project'].__dict__
        obj = json.loads(str(project_obj).replace("'", "\"").replace("True", '"True"').replace("False", '"False"'))
        payload = f"{payload} > 阿明: {obj} \n\n"
