...
from intentionet.bfe.proto.api_gateway_pb2 import ListPoliciesRequest
...
class Session(BaseSession):
    ...
    def _wait_for_snapshot_with_name(self, network: str, snapshot_name: str) -> None:
        """
        Wait for snapshot with specified name to reach READY state

        Deprecated. This is for compatibility for BFE backends older than 2022.04.25.2513
        """
        self._wait_for_snapshot_status_ready(
            lambda: self._get_snapshot_metadata_with_name(
                network, snapshot_name
            ).status,
            snapshot_name,
        )

    def _wait_for_snapshot_not_found_with_name(
        self, network: str, snapshot_name: str
    ) -> None:
        """
        Wait for snapshot with specified name to reach NOT_FOUND state

        Deprecated. This is for compatibility for BFE backends older than 2022.04.25.2513
        """
        self._wait_for_snapshot_status_not_found(
            lambda: self._get_snapshot_metadata_with_name(
                network, snapshot_name
            ).status,
            snapshot_name,
    ...
    def _wait_for_snapshot(
        self, network: str, snapshot_id: str, snapshot_name: str
    ) -> None:
        """Wait for snapshot to reach READY state"""
        self._wait_for_snapshot_status_ready(
            lambda: self._get_snapshot_metadata(
                network, snapshot_id, snapshot_name
            ).status,
            snapshot_name,
        )

    def _wait_for_snapshot_not_found(
        self, network: str, snapshot_id: str, snapshot_name: str
    ) -> None:
        """Wait for snapshot to reach NOT_FOUND state"""
        self._wait_for_snapshot_status_not_found(
            lambda: self._get_snapshot_metadata(
                network, snapshot_id, snapshot_name
            ).status,
    ...
    def _wait_for_snapshot_status_ready(
        self,
        get_current_status: Callable[[], "SnapshotStatus"],
        snapshot_name: str,
    ) -> None:
        """Wait until the snapshot status becomes READY"""
        from intentionet.bfe.proto.api_gateway_pb2 import SnapshotStatus

        done = False
        while not done:
            current_status = get_current_status()
            if current_status == SnapshotStatus.SNAPSHOT_STATUS_READY:
                return
            if current_status in [
                SnapshotStatus.SNAPSHOT_STATUS_DELETING,
                SnapshotStatus.SNAPSHOT_STATUS_NOT_FOUND,
            ]:
                raise ValueError("Unknown snapshot: {}".format(snapshot_name))
            if current_status == SnapshotStatus.SNAPSHOT_STATUS_FAILED:
                raise Exception(
                    "Snapshot {} is failed and will not become ready".format(
                        snapshot_name
                    )
                )

            assert current_status == SnapshotStatus.SNAPSHOT_STATUS_PENDING
            # TODO smarter sleeping
            time.sleep(1)  # seconds
            continue

    def _wait_for_snapshot_status_not_found(
        self, get_current_status: Callable[[], "SnapshotStatus"], snapshot_name: str
    ) -> None:
        """Wait until the snapshot status becomes NOT_FOUND"""
        from intentionet.bfe.proto.api_gateway_pb2 import SnapshotStatus
        ...
        while not done:
            ...
            if current_status == SnapshotStatus.SNAPSHOT_STATUS_FAILED:
                raise Exception(
                    "Snapshot {} is failed and will not be deleted".format(
                        snapshot_name
                    )
                )
            if current_status == SnapshotStatus.SNAPSHOT_STATUS_PENDING:
                raise Exception(
                    "Snapshot {} is pending and will not be deleted".format(
                        snapshot_name
                    )
                )
            if current_status == SnapshotStatus.SNAPSHOT_STATUS_READY:
                raise Exception(
                    "Snapshot {} is ready and will not be deleted".format(snapshot_name)
                )

            assert current_status == SnapshotStatus.SNAPSHOT_STATUS_DELETING
            # TODO smarter sleeping
            time.sleep(1)  # seconds
            continue
    ...
    def fork_snapshot(
        ...
        resp = self._api_gw.ForkSnapshot(
            ForkSnapshotRequest(
                credentials=self._credentials,
                network_name=network,
                snapshot_name=name,
        ...
        if resp.snapshot_id:
            self._wait_for_snapshot(network, resp.snapshot_id, name)
        else:
            self._wait_for_snapshot_with_name(network, name)
        self.snapshot = name
        return name
    ...
    def _experimental_init_partial_snapshot(
        ...
        resp = self._api_gw.ForkFromMasterSnapshot(request)
        if resp.snapshot_id:
            self._wait_for_snapshot(network, resp.snapshot_id, name)
        else:
            self._wait_for_snapshot_with_name(network, name)
    ...
    def _initialize_snapshot(
        ...
        if isinstance(upload, str):
            ...
            snapshot_id = self._init_snapshot_from_file(
            ...
            snapshot_id = self._init_snapshot_from_io(
                name, upload, debug_flags, disable_parse_reuse
            )

        if snapshot_id:
            self._wait_for_snapshot(network, snapshot_id, name)
        else:
            self._wait_for_snapshot_with_name(network, name)
    ...
    def _init_snapshot_from_file(
        ...
        """
        Initialize snapshot with a given path to a zip file or a directory

        :return: ID of the snapshot or None if the InitSnapshotResponse doesn't contain the ID
        """
        if os.path.isdir(file_to_send):
            data = zip_from_dir(file_to_send)
            return self._init_snapshot_from_io(
                name, data, debug_flags, disable_parse_reuse
            )
            ...
            with open(file_to_send, "rb") as fd:
                return self._init_snapshot_from_io(
                    name, fd, debug_flags, disable_parse_reuse
                )
        else:
    ...
    def _init_snapshot_from_io(
        ...
        """
        Initialize snapshot with a given zip file
        ...
        resp = self._api_gw.InitSnapshot(req)
        snapshot_id: Optional[str] = resp.snapshot_id if resp.snapshot_id else None
        return snapshot_id
    ...
    def _get_snapshot_metadata_with_name(
        self, network: str, snapshot_name: str
    ) -> "SnapshotMetadataMessage":
        """
        Returns the current status of the specified snapshot with snapshot name
        """
        from intentionet.bfe.proto.api_gateway_pb2 import (
            SnapshotMetadata as SnapshotMetadataMessage,
        )
        ...
        # It uses ListSnapshotMetadata to get SnapshotMetadata instead of using GetSnapshotMetadata
        # since GetSnapshotMetadata is unreliable on the 2022.04.25.2513 release.
        # (GetSnapshotMetadata throws an UNKNOWN exception(instead of returning NOT_FOUND) for non-existent snapshot)
        for snapshot_metadata in self._list_snapshot_metadata(network):
            if snapshot_metadata.snapshot_name == snapshot_name:
                return snapshot_metadata

        return SnapshotMetadataMessage(status=SnapshotStatus.SNAPSHOT_STATUS_NOT_FOUND)

    def _get_snapshot_metadata(
        self, network: str, snapshot_id: str, snapshot_name: str
    ) -> "SnapshotMetadataMessage":
        """
        Returns the current status of the specified snapshot with snapshot id
        """
        from intentionet.bfe.proto.api_gateway_pb2 import (
            SnapshotMetadata as SnapshotMetadataMessage,
        )
        ...
        # (GetSnapshotMetadata throws an UNKNOWN exception(instead of returning NOT_FOUND) for non-existent snapshot)
        for snapshot_metadata in self._list_snapshot_metadata(network):
            if snapshot_metadata.snapshot_id == snapshot_id:
                return snapshot_metadata
    ...
    def delete_snapshot(self, name: str) -> None:
        ...
        snapshot_metadata = self._get_snapshot_metadata_with_name(self.network, name)
        if snapshot_metadata.status == SnapshotStatus.SNAPSHOT_STATUS_NOT_FOUND:
            raise ValueError("Snapshot {} was not found.".format(name))

        if snapshot_metadata.snapshot_id:
        ...
        if snapshot_metadata.snapshot_id:
            self._wait_for_snapshot_not_found(
                self.network, snapshot_metadata.snapshot_id, name
            )
        else:
            self._wait_for_snapshot_not_found_with_name(self.network, name)
