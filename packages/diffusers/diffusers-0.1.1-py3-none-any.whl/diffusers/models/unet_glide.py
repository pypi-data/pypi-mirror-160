...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if up:
            self.h_upd = Upsample2D(channels, False, dims)
            self.x_upd = Upsample2D(channels, False, dims)
        elif down:
            self.h_upd = Downsample2D(channels, False, dims)
            self.x_upd = Downsample2D(channels, False, dims)
        else:
            ...
            if level != len(channel_mult) - 1:
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ...
                        if resblock_updown
                        else Downsample2D(ch, conv_resample, dims=dims, out_channels=out_ch)
                    )
                )
                ch = out_ch
                input_block_chans.append(ch)
            ...
            for i in range(num_res_blocks + 1):
                ...
                if level and i == num_res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else Upsample2D(ch, conv_resample, dims=dims, out_channels=out_ch)
                    )
                    ds //= 2
