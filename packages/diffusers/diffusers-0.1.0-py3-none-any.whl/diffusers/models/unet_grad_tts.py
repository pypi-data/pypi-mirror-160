...
class ResnetBlock2D2D(torch.nn.Module):
    def __init__(self, dim, dim_out, time_emb_dim, groups=8):
        super(ResnetBlock2D2D, self).__init__()
    ...
    def __init__(self, dim, dim_mults=(1, 2, 4), groups=8, n_spks=None, spk_emb_dim=64, n_feats=80, pe_scale=1000):
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                torch.nn.ModuleList(
                    [
                        ResnetBlock2D2D(dim_in, dim_out, time_emb_dim=dim),
                        ResnetBlock2D2D(dim_out, dim_out, time_emb_dim=dim),
                        Residual(Rezero(LinearAttention(dim_out))),
                        Downsample2D(dim_out) if not is_last else torch.nn.Identity(),
                    ]
                )
            )

        mid_dim = dims[-1]
        self.mid_block1 = ResnetBlock2D2D(mid_dim, mid_dim, time_emb_dim=dim)
        self.mid_attn = Residual(Rezero(LinearAttention(mid_dim)))
        self.mid_block2 = ResnetBlock2D2D(mid_dim, mid_dim, time_emb_dim=dim)

        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            self.ups.append(
                torch.nn.ModuleList(
                    [
                        ResnetBlock2D2D(dim_out * 2, dim_in, time_emb_dim=dim),
                        ResnetBlock2D2D(dim_in, dim_in, time_emb_dim=dim),
