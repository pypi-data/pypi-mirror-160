...
def generate_transformer_pipeline(
    num_attribs: list, cat_attribs: list, cat_labels: list
) -> ColumnTransformer:
...
def split_transform_data(download_path: str) -> tuple[str, str]:
    ...
    prepared_columns = sklearn_utils.get_feature_names_from_column_transformer(
        transformer_pipeline
    )
    housing_prepared = pd.DataFrame(
        housing_prepared_values, housing.index, prepared_columns
    )
    ...
    X_test_prepared = pd.DataFrame(
        X_test_prepared_values, X_test.index, prepared_columns
    )
    ...
    logger.info(
        f"Created new features, now there are {train_dataset.shape[1]} features"
    )
