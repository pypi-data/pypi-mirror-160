...
from src.k8s_utils import get_all_pbs_daemonset, delete_pbs_daemonset, get_all_pblog_daemonset, get_services
from src.utils import Error, print_error_body, clean_stderr
...
def launch_pods(name: str, path: str, api_client: ApiClient) -> bool:
    with Dir(path):
        ...
        ret = subprocess.run(cmd.split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=30)
...
def cutover_service(name: str, path: str, api_client: ApiClient) -> Status:
    ...
    with Dir(path):
        ...
        terminal_menu = TerminalMenu(["[y] yes", "[n] no, let's wait"], title=f"Should we cutover?",
        ...
        ret = subprocess.run(cmd.split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=30, input='y')
        ret.stderr = clean_stderr(ret.stderr)
        if ret.returncode != 0 or ret.stderr != "":
            print_error_body(f"ERROR:\n\nstdout:\n{ret.stdout}\nstderr:\n{ret.stderr}")
            return Status.FAIL
