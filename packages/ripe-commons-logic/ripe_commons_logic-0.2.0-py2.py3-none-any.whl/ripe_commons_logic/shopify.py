...
class ShopifyCommons(object):
    ...
    def build_shopify_product(cls, **kwargs):
        ...
        options = []
        show_size = utils.try_execute_method(
            "show_shopify_size", fallback=cls.show_shopify_size, **kwargs
        )
        show_scale = utils.try_execute_method(
            "show_shopify_scale", fallback=cls.show_shopify_scale, **kwargs
        )
        show_gender = utils.try_execute_method(
            "show_shopify_gender", fallback=cls.show_shopify_gender, **kwargs
        )
        if show_size and size_scaled:
            options.append(dict(name="Size"))
            customized_variant["option%d" % len(options)] = size_scaled

        if show_scale and scale:
        ...
        return dict(
            ...
            metafields=[
                dict(namespace="seo", key="hidden", value=1, value_type="integer"),
                dict(
                    namespace="platforme",
                    key="context",
                    value=json.dumps(
    ...
    @classmethod
    def show_shopify_size(cls, **kwargs):
        return True

    @classmethod
    def show_shopify_scale(cls, **kwargs):
        return True

    @classmethod
    def show_shopify_gender(cls, **kwargs):
        return True
