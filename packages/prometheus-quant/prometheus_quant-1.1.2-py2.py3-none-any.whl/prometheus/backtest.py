...
class template(callback):
    ...
    def __init__(self):
        ...
        # 获取行情数据的工作子线程
        self.__get_quote_data_subprocess = None
    ...
    def run(self):
        """回测启动运行"""
        # 初始化所有的策略
        self.__init_all_strategy()

        # 启动所有策略，并设置回调对象
        self.__start_all_strategy()

        # 自动订阅所有的标的，并记录
        self.__subscribe_all_symbol_and_record()
    ...
    def __do_backtest(self):
        """执行回测"""

        # 启动子进程，从预设的数据源中逐个获取数据，并放入队列中
        self.__fetch_quote_data()

        # 模拟行情播放
        self.__simulate_broadcast_quote_data()

    def __fetch_quote_data(self):
        """启动子进程，从预设的数据源中逐个获取数据，并放入队列中"""
        self.__get_quote_data_subprocess = Process(
                target=self.subprocess_get_quote_data, 
                args=(self.__queue_quote_source, )
            )
        self.__get_quote_data_subprocess.start()
    ...
    def __simulate_broadcast_quote_data(self):
        """执行回测，不断从队列中读取数据，进行播放，直到队列为空，且子进程关闭"""
        index = 0
        while self.__get_quote_data_subprocess.is_alive() or not self.__queue_quote_source.empty():
            # 如果队列为空，重新判断
