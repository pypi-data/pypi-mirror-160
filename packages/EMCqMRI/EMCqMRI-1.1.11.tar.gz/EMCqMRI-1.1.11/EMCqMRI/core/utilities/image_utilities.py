...
def saveData(data, path, filename, file_extension):
    data['estimated_params'] = np.moveaxis(data['estimated_params'].detach().numpy(), 0, -1)
    data['signal'] = np.moveaxis(data['signal'].detach().numpy(), 0, -1)
    if 'predicted_signal' in data.keys():
        data['predicted_signal'] = np.moveaxis(data['predicted_signal'].detach().numpy(), 0, -1)
    ...
    if file_extension=='.nii':
        ...
        logging.info("Writing file {} to {}".format(filename + '_parameters.nii', path))
        nib_est_params = nib.Nifti1Image(data['estimated_params'], affine=np.eye(4))
        nib.save(nib_est_params, path_params)

        logging.info("Writing file {} to {}".format(filename + '_measured_signal.nii', path))
        nib_signal = nib.Nifti1Image(data['signal'], affine=np.eye(4))
        nib.save(nib_signal, path_signal)
        if 'predicted_signal' in data.keys():
            logging.info("Writing file {} to {}".format(filename + '_fitted_signal.nii', path))
            nib_est_signal = nib.Nifti1Image(data['predicted_signal'], affine=np.eye(4))
        ...
        with open(path_params, 'wb') as f:
            logging.info("Writing file {} to {}".format(filename + '_parameters.pkl', path))
            pickle.dump(data['estimated_params'].detach().numpy(), f)
        with open(path_signal, 'wb') as f:
            logging.info("Writing file {} to {}".format(filename + '_measured_signal.pkl', path))
            pickle.dump(data['signal'].detach().numpy(), f)
        if 'predicted_signal' in data.keys():
            with open(path_est_signal, 'wb') as f:
                logging.info("Writing file {} to {}".format(filename + '_fitted_signal.pkl', path))
                pickle.dump(data['predicted_signal'].detach().numpy(), f)

def loadData(data, path, filename, file_extension):
    #Implement (requires some refactoring)
    pass
