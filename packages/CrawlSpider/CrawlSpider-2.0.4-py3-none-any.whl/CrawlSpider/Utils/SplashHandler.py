...
class WebSplash:
    ...
    def connect(self, options:list=None, page_load_timeout=60, script_timeout=60, chrome_debugger_port=9224):
        debugger_address = self.launch_chrome(chrome_debugger_port=chrome_debugger_port)
        driver = self.getNewDriver(options=options, debugger_address=debugger_address)
