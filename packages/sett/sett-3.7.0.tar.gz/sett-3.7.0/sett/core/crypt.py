...
def verify_key_length(key: gpg.Key, min_key_length: int = 4096) -> None:
    try:
        ...
        if key.key_length < min_key_length:
            raise UserError(
                f"Not allowed: key {key.key_id} is using an algorithm which we do not allow. "
                f"Use either the RSA algorithm (min. key length: {min_key_length}) "
                "or the ECC algorithm instead."
            ) from exc
    if algorithm == KeyAlgorithm.ECC:
        msg = (
            f"Please be aware: key {key.key_id} is of type ECC, which is not supported "
            "by older versions of GnuPG, available on older operating systems."
        )
        warnings.warn(msg)
        return
    if algorithm == KeyAlgorithm.DSA:
        raise UserError(
            f"Not allowed: key {key.key_id} is of type DSA, which we do not allow."
        )
    if key.key_length < min_key_length:
        raise UserError(
            f"Not allowed: key {key.key_id} is shorter than the minimal "
