...
class ManagerInterp(PlotPanel):
    ...
    def tft_vec(self,zonename):
        ...
        self.active_vec.parent = curzone
        ...
        if curzone.nbvectors>0:
            self.active_vec=curzone.myvectors(min(curzone.nbvectors,self.curidx))
        pass
    ...
    def plot_curvec(self):
        ...
        self.myax.clear()
        self.figure.set_title(curvec.myname)

        self.myax.plot(s,z)
        self.ax_3D.plot(curvec.asnparray3d())
        ...
        for curcs in self.active_cs.myprofiles.values():
            ...
            if curvec.linestring.intersects(curprof.linestring):
                ...
                self.zprof.append = curprof.interpolate(curprof.linestring.project(inter),adim=False).z
                
                self.ax_3D.plot(curprof.asnparray3d())
    ...
    def corrz(self,event):
        ...
        ls = LineString(asarray([[s,z] for s,z in zip(self.sprof,self.zprof)]))
    ...
    def plot_down(self,event):
        self.curidx+=1
        self.curidx=max(self.curidx,self.nbvectors)
