...
class dictionary:
    ...
    def get_elements(self, position, **kwargs):
        keys = list({k for d in self.input for k in d.keys()})
        values = list({k for d in self.input for k in d.values()})
        if position == "k":
            return keys
        elif position == "v":
            return values
    ...
    def keys_filtering(self, filter_list):
        ...
        return list([list(item.values())[0] for item in self.input if list(item.keys())[0] in filter_list])
    ...
    def formatting(self,formatting_type,**kwargs):
        ...
        if formatting_type== "bq_conversion":
            ...
            if type(self.input) != list and type(schema_master) == list:
        ...
        if type(self.input) == list:
            ...
            if formatting_type == "types_parsing":
                if type(self.input[0]) == dict:
                    ...
                    rows_union=[]
                    for idx,row in enumerate(self.input):
                            row_formatted = dictionary.formatting(row, formatting_type, **kwargs)
                ...
                if v in [None,{},'None']:
                    ...
                    if formatting_type == "bq_conversion":
                        if type(schema_master) == list:
                            if type(input) != list:
                                output = dictionary.formatting([self.input], formatting_type, schema=schema_master[0])
                            else:
                                output = dictionary.formatting(self.input, formatting_type, schema=schema_master[0])
                        else:
                            try:
            ...
            if formatting_type == "bq_conversion" and schema_master != type(input).__name__:
                ...
                output = dictionary.type_converting(self.input,manual_type=output_type)
            elif formatting_type == "types_parsing":
                output = type(self.input).__name__
            elif formatting_type == "custom_conversion":
                    output=kwargs["conversion_dict"][self.input]
            else:
                output = self.input
...
class lists:
    ...
    def join_string(self,separator=','):
        ...
        if len(self.list_object) == 1:
            return eval(self.list_object[0])
        else:
            return separator.join([str(eval(i)) for i in self.list_object])

    def clean_currency(self):
        return self.list_object.str.replace(",", ".").str.replace("Kč", "").str.replace(" ", "").astype(float)

    def remove_duplicates(self):
        return list(set([i for i in self.list_object]))


    def get_element_by_name(self,name):
        element=[i for i in self.list_object if name in i["name"]][0]
        return element
...
class time:
    ...
    def iso(date_input):
        ...
        output= tz.localize(datetime.combine(date_input, time())).astimezone(pytz.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
        return output
    ...
    def today_plus(granularity="datetime",dplus=0,out_format="string"):
        date_add = (dtd.now() + timedelta(days=dplus))
