...
class TurnoverProp(FactorLiquidity):
    ...
    def calculate(self, date):
        short_last_date = pd.date_range(end=date, periods=self.short_period, freq=ba.CBD)[0]
        long_last_date = pd.date_range(end=date, periods=self.long_period, freq=ba.CBD)[0]
...
class AmountPerVolume(FactorLiquidity):
    ...
    def calculate(self, date):
        volume_last_date = pd.date_range(end=date, periods=self.period, freq=ba.CBD)[0]
        price_last_date = pd.date_range(end=date, periods=self.calculate_period, freq=ba.CBD)[0]
        ...
        self.factor = np.log((volume / volatility).groupby(level=1).mean())
...
class AmountPerChangeMA(FactorLiquidity):
    ...
    def calculate(self, date):
        ...
        change = np.abs(self.datasource.market_daily(last_date, date, fields='adj_pct_change'))
        self.factor = np.log((volume / change).groupby(level=1).mean())\
            .replace(np.inf, np.nan).replace(-np.inf, np.nan).dropna()
