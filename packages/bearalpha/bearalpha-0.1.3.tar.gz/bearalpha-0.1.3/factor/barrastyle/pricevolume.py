...
class HAlpha(FactorPriceVolume):
    ...
    def calculate(self, date):
        last_date = pd.date_range(end=date, periods=self.period, freq=ba.CBD)[0]
...
class ExpWeightedMomentum(FactorPriceVolume):
    ...
    def calculate(self, date):
        last_date = pd.date_range(end=date, periods=self.period + 1, freq=ba.CBD)[0]
        ...
        self.factor = pd.concat([change, turnover], axis=1)\
            .groupby(level=1).apply(lambda x: 
                ...
                if len(x) == self.period + 1 else np.nan)
...
class PriceVolumeMeasuredSentiment(FactorPriceVolume):
    ...
    def calculate(self, date):
        rolling_window = pd.date_range(end=date, periods=self.period, freq=ba.CBD)
        ...
        yeild_multiple_vector = (np.logical_or((yeild_matrix>0).all(),(yeild_matrix<0).all())*self.multiple).replace(0,1).values
        volume_multiple_vector = (np.logical_or((volume_diff_matrix>0).all(),(volume_diff_matrix<0).all())*self.multiple).replace(0,1).values
...
class ResidualMomentum(FactorPriceVolume):
    ...
    def calculate(self, date):
        ...
        Y = change
        X = pd.concat([rm, HML, SMB],axis=1)
        regresult = Y.groupby(level=1).apply(lambda x: X.regressor.ols(y=x.droplevel(1)) if len(x) == self.period else np.nan)
        self.factor = regresult.apply(lambda x: x.raw_result.resid.abs().sum() if (not pd.isna(x)) else np.nan)
...
class AR(FactorPriceVolume):
    ...
    def calculate(self, date):
        ...
        self.factor = ((data.high - data.open)/(data.open - data.low)).replace(np.inf, 0).groupby(level=1).sum()
