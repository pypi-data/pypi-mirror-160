...
def match_files_by_name(file_type_dict: Mapping[str, List[str]]) -> Mapping[str, Mapping]:
    ...
    for fi_idx, fi in enumerate(file_type_dict[fi_start]):
        ...
        for fi_type in fil_types:
            fi_path_list = (
                [file_type_dict[fi_type]] if not isinstance(file_type_dict[fi_type], list) else file_type_dict[fi_type]    
            )
            fi_top = ""
            seq_top = 0.0
...
def get_input_files_from_prefix(
    ...
    print(f"The following datasets/groups are specified as inputs: {input_files.keys()}")
    return input_files
