...
def build_bench_sparse_count_nonzero(params):
  ...
  sp_tensors = [
    tf.sparse.from_dense(
      vals[c])
    for c in xrange(params.num_columns)]
  if not params.dense_count:
    if params.num_columns == 1:
      bench_ops = [
        hb.sparse.count_nonzero(
          sp_tensors[0], axis=-1, dtype=tf.int64)]
    else:
      bench_ops = [
        hb.sparse.count_nonzero_n(
          sp_tensors, axis=-1, dtype=tf.int64)]
  elif params.reorder:
    bench_ops = [
...
if __name__ == '__main__':
  ...
  parser.add_argument('--dense-count', default=False, action='store_true')
