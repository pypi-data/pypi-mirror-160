...
def load_(file, return_text=False):
...
#				print(each,mcfg[each])

#			print('mcfg[outfile]',mcfg['outfile'])
#			print('cfg[outfile]',cfg['outfile'])
...
def get_parameter_sets(pars=sys.argv[1:]):
	...
	ext ='(json|json5)'
	expr="^[^-{].*\."+ext+"$"
	# The JSON5 format is used to feed the input parameters
	# since it is easy to convert to a python dictionary.
	# Only check for JSON and JSON5 files for now:
	...
	# well: e.g., yaml?
	# Note the format without the outer most brackets '{',''}' in JSON5 is
	...
	# To avoid this, use the "JSON5" extension.
...
#		fups = list(set(cfg['-filecheck']) & set(prep['-main<<prep']))
	...
	for task in tasks:
		...
		for key, lmcfg in mcfg.items(): 
			...
			if   cfgs[key]['-native'] == native      : sorted_kwargs[key][native_key] =  lmcfg
