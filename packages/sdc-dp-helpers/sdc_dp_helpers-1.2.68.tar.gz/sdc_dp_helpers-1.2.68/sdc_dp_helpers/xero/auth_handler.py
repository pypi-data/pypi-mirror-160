...
def refresh_auth_token(client_id: str, auth_creds: OAuth2Credentials, local_token_path: str) -> OAuth2Credentials:
    cred = {
    ...
    if not token_isvalid(auth_creds):
        raise XeroForbidden(f"An error occured while trying to authenticate the refreshed token: {str(auth_creds)}")
    return auth_creds
