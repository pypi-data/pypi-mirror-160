...
#For ref optional, ospath initalize
...
class _RefGetterSetter(_GlobalRefPrint) :
    ...
    def set_ospath(self, path) :
        _GlobalRefPrint.ospath = path
...
class ConnxModelProto(ConnxObject):
    def __init__(self, proto, arg_ref, ospath):
        super().__init__(proto)

        self.next_graph_id = 0

        #set ref value, ospath
        refClass = _RefGetterSetter()
        refClass.set_ref_print(arg_ref)
        refClass.set_ospath(str(ospath).replace('/model.onnx','')) # TODO : if onnx model name is not model.onnx...
...
class ConnxAttributeProto(ConnxObject):
    ...
    def compile(self, out):
        os_path = _GlobalRefPrint.ospath
        if self.proto.type == 0:
            ...
            if self.proto.type in [1, 2]:
                ...
                out.write(' ')
                out.write(s)
            elif self.proto.type == 4 :
                out.write(str(self.proto.type))
                out.write(' ')
                out.write(str(self.proto.t.data_type))
                out.write(' ')
                
                #FLOAT32
                if self.proto.t.data_type == 1:
                    dim = self.proto.t.dims
                    if len(dim) == 0 :
                        out.write('1')
                        out.write(' ')
                        out.write('1')
                    else :
                        out.write(str(len(dim)))
                        for i in range(len(dim)) :
                            out.write(' ')
                            out.write(str(dim[i]))
                    value = self.proto.t.float_data
                    if len(value) == 0:
                        value = self.proto.t.raw_data
                        value = numpy.frombuffer(value, dtype = numpy.float32)
                    for j in range(len(value)) :
                        out.write(' ')
                        out.write(str(value[j]))

                #INT32
                elif self.proto.t.data_type == 6:          
                    dim = self.proto.t.dims
                    if len(dim) == 0 :
                        out.write('1')
                        out.write(' ')
                ...
                # STRING
                # elif self.proto.t.data_type == 8:       
                #     s = self.proto.t.string_data[0].decode('utf-8')
                #     out.write(str(len(str(s))))
                #     out.write(' ')
                #     out.write(s) 
                # elif self.proto.t.data_type == 8:                     
                #     value = self.proto.t.string_data
            ...
            elif self.proto.type == 5 :
                out.write(str(self.proto.type))
                ConnxGraphProto(self.proto.g, self).compile(path = os_path)
            elif self.proto.type in [6, 7]:
                out.write(str(self.proto.type))
                out.write(' ')
                value = self.value()
                if self.proto.type == 7 :
                    if len(value) == 0 :
                        out.write(str(1))
                        out.write(' ')
                    for i in range(len(value)) :
                        if value[i] > 2147483647 or value[i] < -2147483648: # INT MAX
                            out.write(str(0))
                            out.write(' ')
                            break
                        else :
    ...
    def value(self):
        ...
        if type == 1:
            ...
            return ConnxTensorProto(self.proto.t, self)
        elif type == 5:
            ...
            return ConnxSparseTensorProto(self.proto.sparse_tensor, self)
        elif type == 6:
...
class ConnxNodeProto(ConnxObject):
    def __init__(self, proto, parent):
        ...
        root = self.get_root()
        # if 'Softmax' in proto.op_type:
        #     proto.op_type.strip("Softmax")
