...
@cli.group()
@click.pass_context
def secrets(ctx: click.Context):
    # Shared prerequisites for all secrets commands
    _ = require_databutton_login_info()
    config = get_databutton_config()
    ctx.obj = SecretsApiClient(config.uid)
...
def add(ctx: click.Context, key: str, data_file):
    secrets_client: SecretsApiClient = ctx.obj

    # Always reading datafile as bytes but assuming it holds
    # a utf8 encoded string so validate that early here,
    # as well as a max length.
    max_len = 500
    ...
    # Send secret string to cloud
    success = secrets_client.add(key, value)
    if success:
...
def delete(ctx: click.Context, key: str):
    ...
    success = secrets_client.delete(key)
