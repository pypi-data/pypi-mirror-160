...
class XRedis(Singleton):
    def __init__(self, host, password, port, db=0, max_connections=4):
    ...
    def _make_conn(self):
        try:
            ...
            pool = redis.ConnectionPool(
                host=self.host, password=self.password, port=self.port, db=self.db, decode_responses=True, max_connections=self.max_connections, socket_timeout=60, socket_connect_timeout=30, socket_keepalive=True, retry_on_timeout=True)
...
class XRedisAsync(Singleton):
    def __init__(self, host, password, port, db=0, max_connections=50):
        if not self.need_init():
            return
        self.host = host
        self.password = password
        ...
        pool = coredis.ConnectionPool(host=self.host, password=self.password, port=self.port, db=self.db, decode_responses=True, max_connections=self.max_connections, socket_keepalive=True, retry_on_timeout=True)
