...
class Easyocrpipleline:
    ...
    def create_json(self, result, file):
        ...
        print("/tmp/testfolder/"+json_name,'+++++++++++++++++++')
        with open("/tmp/testfolder/"+json_name+".json", "w") as outfile:
    ...
    def tif_image_process(self, path):
        ...
        for i in range(img.n_frames):
            ...
            if img.n_frames==1:
                ...
                img.seek(i)
                tif_file_path=('/tmp/'+tif_file_name+'(%s).tif'%(i,))
    ...
    def pdf_process(self, path):
        ...
        for index, image in enumerate(images):
            path = '/tmp/'+file_name+'('+str(index)+').jpg'
            image.save(path)
