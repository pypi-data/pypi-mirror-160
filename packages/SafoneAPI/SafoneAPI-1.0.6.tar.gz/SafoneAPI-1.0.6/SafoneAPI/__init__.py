...
class SafoneAPI:
    ...
    def _get_fname(self, format: str) -> str:
        return f"{str(uuid.uuid4().hex)}.{format}".rstrip()
    ...
    async def _parse(self, response):
        try:
            ...
            raise InvalidContent(
                "Content Not Found, Please report this: https://api.safone.tech/report"
            )
        if type in ["txt", "html", "json"]:
            resp = DotMap(await response.json())
            return resp
        elif type in ["jpg", "jpeg", "png", "gif", "webp"]:
            file = BytesIO(await response.read())
            file.name = self._get_fname(type)
            return file
        else:
            raise InvalidContent(
