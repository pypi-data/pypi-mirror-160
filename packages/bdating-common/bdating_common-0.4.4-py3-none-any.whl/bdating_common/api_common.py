...
from bdating_common import model_zh as model_zh_package
...
def get_model_enum_values():
    model_enums= {}
    for data_type_name in dir(model_package):
        if not data_type_name.startswith('__') and data_type_name != 'Enum' and type(model_package.__dict__[data_type_name]) == enum.EnumMeta:
            model_enums[data_type_name] = [
                {
                    '_key': variable_name,
                    'en': model_package.__dict__[data_type_name].__dict__[variable_name].value,
                    'zh': model_zh_package.__dict__[data_type_name].__dict__[variable_name].value,
                }
                for variable_name in dir(model_package.__dict__[data_type_name]) if not variable_name.startswith('__')
            ]
    return model_enums
