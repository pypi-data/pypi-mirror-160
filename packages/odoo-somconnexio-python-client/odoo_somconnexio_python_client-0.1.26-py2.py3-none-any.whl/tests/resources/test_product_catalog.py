...
class TariffTests(unittest.TestCase):
    ...
    def test_search_code(self):
        ...
        adsl_wo_fix_product = list(filter(lambda p: p.code == "SE_SC_REC_BA_ADSL_SF",
                                          pricelist_21IVA.products))
        self.assertEqual(len(adsl_wo_fix_product), 1)
        self.assertFalse(adsl_wo_fix_product[0].has_landline_phone)

        fiber100_with_fix_product = list(filter(lambda p: p.code == "SE_SC_REC_BA_F_100",
                                                pricelist_21IVA.products))
