...
_NON_UPDATEABLE_REASONS = {
    ...
    **{variant: f'Auto-update is not supported for unpackaged {name} executable; Re-download the latest release'
    ...
    'unknown': 'It looks like you installed VIP-yt with a package manager, pip or setup.py; Use that to update',
    'other': 'It looks like you are using an unofficial build of VIP-yt; Build the executable again',
}
...
class Updater:
    ...
    def check_update(self):
        ...
        try:
            ...
            if not self.has_update:
                if self._tag == 'latest':
                    return self.ydl.to_screen(f'VIP-yt is up to date ({__version__})')
                return self.ydl.report_warning(
                    'VIP-yt cannot be updated any further since you are on an older Python version')
        except Exception:
    ...
    def update(self):
        """Update VIP-yt executable to the latest version"""
        ...
        self.ydl.to_screen(f'Updated VIP-yt to version {self.new_version}')
        return True
...
def update_self(to_screen, verbose, opener):
    ...
    write_string(
        'DeprecationWarning: "VIP_yt.update.update_self" is deprecated and may be removed in a future version. '
        'Use "VIP_yt.update.run_update(ydl)" instead\n')
