...
class Program(object):
    def __init__(self, params_sets, model_sets, method, 
                random_state,
                p_point_replace, gen, fitness, 
                model_name=None,
                params=None, program=None,
                parents=None):
        self._id =  str(int(time.time() * 1000000 + datetime.datetime.now().microsecond))
        ...
        if self._program is None:
            self._program = self.build_program(
                random_state, params_sets, model_sets, model_name)
        else:
    ...
    def build_program(
        self, random_state, params_sets, model_sets, model_name):
        ### 选择模型
    ...
    def log(self):
        print("name:{0},gen:{1},params:{2},fitness:{3},method:{4},token:{5}".format(
            self._name, self._gen, self._params, self._raw_fitness,
    ...
    def get_subtree(self, random_state, program=None):
        ...
        probs = np.array([0.9 if params[node] in self._params_sets[node] else 0.1 for node in params.keys()])
    ...
    def point_mutation(self, random_state):
        ...
        mutate = np.where(random_state.uniform(size=len(program['params'])) <
                          self._p_point_replace)[0]
        removed =[list(self._program['params'].keys())[node] for node in mutate]
        remain = list(set(self._program['params'].keys()) - set(removed))
        for key in removed:
            program['params'][key] = random_state.choice(self._params_sets[key])
    ...
    def raw_fitness(self, features, X, Y, mode, n_splits):
        if self._fitness is None:
            raw_fitness = model_fitness(
                features=features,model_name=self._model_name,
                X=X, Y=Y,mode=mode,n_splits=n_splits,
                params=self._params)
        else:
            raw_fitness = self._fitness(features=features,model_name=self._model_name,
                X=X, Y=Y,
                params=self._params)
