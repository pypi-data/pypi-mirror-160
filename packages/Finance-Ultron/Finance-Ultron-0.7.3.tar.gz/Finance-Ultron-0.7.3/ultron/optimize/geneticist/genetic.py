...
def parallel_evolve(n_programs, parents, features, X, Y, seeds, greater_is_better, gen, params):
    ...
    def _tournament(tour_parents, model_name=None):
        if model_name is not None:
            tour_parents = [p for p in tour_parents if p._model_name == model_name]
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            for contender in contenders:
                ...
                parent = Program(
                    params_sets=params_sets, model_sets=model_sets,
                    random_state=random_state,method=init_method,
                    p_point_replace=p_point_replace, gen=gen,
                    fitness=fitness, program=program)
                break
            # 新特征种群
            if random_state.uniform() < method_probs[1]:
                program = Program(
                    ...
                    random_state=random_state,method=init_method,
                    p_point_replace=p_point_replace, gen=gen,
                    fitness=fitness, model_name=parent._model_name)
            ...
            if method < method_probs[0]: # crossover
                donor, donor_index = _tournament(copy.deepcopy(parents), model_name=parent._model_name)
        ...
        program = Program(
                    params_sets=params_sets, model_sets=model_sets,
                    random_state=random_state,method=p_init_method,
                    p_point_replace=p_point_replace, gen=gen,
                    fitness=fitness, program=program)
        #default_value = MIN_INT if greater_is_better else MAX_INT
        program.raw_fitness(
                features=features, X=X, Y=Y, mode=mode, n_splits=n_splits)
        ...
        for gen in range(prior_generations, self._generations):
            ...
            population = Parallel(n_jobs=n_jobs,
                ...
                delayed(parallel_evolve)(n_programs[i], parents, features, X,
                                Y, seeds, self._greater_is_better, gen, params)
