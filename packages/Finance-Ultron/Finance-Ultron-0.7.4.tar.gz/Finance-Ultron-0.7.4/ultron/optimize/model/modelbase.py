...
class ModelBase(metaclass=abc.ABCMeta):
    ...
    def precision(self, x:pd.DataFrame, y: np.ndarray, average='weighted') -> float:
        predict_y = self.impl.predict(x[self.features].values)
        return metrics.precision_score(y, predict_y, average=average)

    def recall(self, x:pd.DataFrame, y: np.ndarray, average='weighted') -> float:
        predict_y = self.impl.predict(x[self.features].values)
        return metrics.recall_score(y, predict_y, average=average)
    
    def roc_auc(self, x:pd.DataFrame, y: np.ndarray, average='weighted') -> float:
        predict_y = self.impl.predict(x[self.features].values)
