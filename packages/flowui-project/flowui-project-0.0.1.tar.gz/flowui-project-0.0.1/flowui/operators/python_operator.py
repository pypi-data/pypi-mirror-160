...
def make_python_callable(operator_name, deploy_mode, task_id, dag_id):
    ...
    input_model_class, output_model_class = load_operator_models_from_path(
        operators_folder_path=operators_folder_path,
        operator_name=operator_name
    )
    return operator_object.run_operator_function, input_model_class, output_model_class
...
class PythonOperator(AirflowPythonOperator):
    ...
    def execute(self, context: Context) -> Any:
        # THIS WAS CHANGED 06/07/2022 to accomodate FlowUI format for calling run_operator_function

        # context_merge(context, self.op_kwargs, templates_dict=self.templates_dict)
        # self.op_kwargs = self.determine_kwargs(context)

        self.context = context
        self.operator_function, self.operator_input_model_class, self.operator_output_model_class = self.python_callable(**self.make_python_callable_kwargs)
