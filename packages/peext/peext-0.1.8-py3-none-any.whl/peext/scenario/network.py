...
def create_district_gas_network():
    ...
    ppipes.create_source(net_gas, junction=jun_id8, mdot_kg_per_s=0.0285, name="BSource 1")
    ppipes.create_source(net_gas, junction=jun_id9, mdot_kg_per_s=0.0285, name="BSource 2")
    ppipes.create_source(net_gas, junction=jun_id10, mdot_kg_per_s=0.01885, name="BSource 3")
    ppipes.create_ext_grid(net_gas, junction=jun_id_ext, p_bar=REF_BAR, t_k=REF_TEMP, name="Grid Connection Gas")
...
def create_district_heat_network(number_of_hh_in_sidelanes=20):
    ...
    jun_id_heat_main_start = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    jun_id_heat_main_start_2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    ppipes.create_heat_exchanger(net_heat, from_junction=jun_id_heat_main_start_2, to_junction=jun_id_heat_main_start, diameter_m=0.075, qext_w=-0.1)

    jun_id_heat_main_mid1 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    jun_id_heat_main_mid1_2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    ppipes.create_heat_exchanger(net_heat, from_junction=jun_id_heat_main_mid1, to_junction=jun_id_heat_main_mid1_2, diameter_m=0.075, qext_w=-0.1)
    
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_start, to_junction=jun_id_heat_main_mid1, length_km=4,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_mid1_2, to_junction=jun_id_heat_main_start_2, length_km=4,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  

    jun_id_heat_main_mid2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    jun_id_heat_main_mid2_2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    ppipes.create_heat_exchanger(net_heat, from_junction=jun_id_heat_main_mid2_2, to_junction=jun_id_heat_main_mid2, diameter_m=0.075, qext_w=-0.1)
    
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_mid1, to_junction=jun_id_heat_main_mid2, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_mid2_2, to_junction=jun_id_heat_main_mid1_2, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  

    jun_id_heat_main_end = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    jun_id_heat_main_end_2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
    ppipes.create_heat_exchanger(net_heat, from_junction=jun_id_heat_main_end_2, to_junction=jun_id_heat_main_end, diameter_m=0.075, qext_w=-0.1)
    
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_mid2, to_junction=jun_id_heat_main_end, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  
    ppipes.create_pipe_from_parameters(net_heat, from_junction=jun_id_heat_main_end_2, to_junction=jun_id_heat_main_mid2_2, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)

    for junc_start_point in [(jun_id_heat_main_start, jun_id_heat_main_start_2), 
        ...
        for _ in range(number_of_hh_in_sidelanes):
            next_jun_1 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
            next_jun_2 = ppipes.create_junction(net_heat, pn_bar=HEAT_BAR, tfluid_k = HEAT_TEMP)
            ppipes.create_pipe_from_parameters(net_heat, from_junction=connect_1, to_junction=next_jun_1, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  
            ppipes.create_pipe_from_parameters(net_heat, from_junction=next_jun_2, to_junction=connect_2, length_km=3,
                            diameter_m=0.105, k_mm=.025, alpha_w_per_m2k=100, sections = 5, text_k=HEAT_TEMP)  
            ppipes.create_heat_exchanger(net_heat, from_junction=next_jun_1, to_junction=next_jun_2, diameter_m=0.075, qext_w=-0.1)
            connect_1 = next_jun_1
            connect_2 = next_jun_2

    ppipes.create_ext_grid(net_heat, junction=jun_id_heat_main_end, p_bar=HEAT_BAR, t_k=HEAT_TEMP)

    return net_heat

def create_super_district_coupled_network(coupling_density, heat_sidelane_size=20):
    net_power = pandapowernetworks.create_synthetic_voltage_control_lv_network("village_2")
    net_power.name = 'power'
    ...
    for i in range(min(len(net_power.load), len(net_gas.source))):
        if random.random() < coupling_density:
            RegulatedP2GControlMultiEnergy(mn, int(random.random()*len(net_power.load)), 
                                               int(random.random()*len(net_gas.source)), 
                                               efficiency = 0.94,
                                               name = f"P2G-{i}")
    # CHP
    for i in range(min(len(net_power.sgen), len(net_gas.sink), len(net_heat.heat_exchanger))):
        if random.random() < coupling_density:
            CHPControlMultiEnergy(mn, int(random.random()*len(net_power.sgen)), 
    ...
    for i in range(min(len(net_power.load), len(net_heat.heat_exchanger))):
        if random.random() < coupling_density:
            RegulatedP2HControlMultiEnergy(mn, int(random.random()*len(net_power.load)), 
                                               int(random.random()*len(net_heat.heat_exchanger)), 
