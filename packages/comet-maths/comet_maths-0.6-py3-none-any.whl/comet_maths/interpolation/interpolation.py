...
def interpolate_1d(x_i,y_i,x,method="cubic",u_y_i=None,min_scale=0.3,return_uncertainties=False,add_error=False,return_corr=False,extrapolate="nearest"):
    ...
    """
    if method.lower()=="gpr":
        return gaussian_process_regression(x_i, y_i, x, min_scale=min_scale, u_y_i=u_y_i, return_uncertainties=return_uncertainties,add_error=add_error,return_corr=return_corr)
...
def gaussian_process_regression(x_i, y_i, x,u_y_i=None,kernel="RBF",min_scale=0.01,max_scale=10000,return_uncertainties=True,add_error=False,return_corr=False):
