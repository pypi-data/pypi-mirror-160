...
class DownloadNotebooksTask(WorkflowTask, IDatabricksTask):
    def __init__(self, manager: IDatabricksWorkspaceManager):
        WorkflowTask.__init__(self)
        self.manager = manager
    ...
    @property
    def task_name(self) -> str:
        return "download_notebooks"

    def run(self, request: DatabricksState) -> any:
        tmpdir = tempfile.gettempdir()
        download_notebooks_local_path = f'{tmpdir}/databricks_notebooks_backup' if not request.download_notebooks_local_path else request.download_notebooks_local_path
        notebook_target_path = '/' if not request.notebook_target_path else request.notebook_target_path
        logger.info(f"Downloading Databricks Notebooks to {download_notebooks_local_path} from {notebook_target_path}")
        self.manager.download(request.client, DatabricksNotebook(source_path=notebook_target_path, target_path=download_notebooks_local_path))