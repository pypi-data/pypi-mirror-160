...
IOU_CACHE: LRUCache = LRUCache(maxsize=1)
...
def _iou_assignments_for_same_reference_id(
    ...
    cache_key = f"{reference_id}_{iou_threshold}_{len(annotations)}_{len(predictions)}"
    if cache_key in IOU_CACHE:
        return IOU_CACHE[cache_key]
    else:
        # Convert annotation and predictions to shapely.geometry.Polygon objects
        polygon_annotations = list(
            map(polygon_annotation_to_shape, annotations)
        )
        polygon_predictions = list(
            map(polygon_annotation_to_shape, predictions)
        )
        ...
        IOU_CACHE[cache_key] = (iou_assigns, anno_to_pred, pred_to_anno)
