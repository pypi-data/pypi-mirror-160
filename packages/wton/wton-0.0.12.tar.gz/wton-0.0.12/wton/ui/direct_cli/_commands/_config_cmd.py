...
from wton.config import update_config_field, unset_config_field, get_config, get_configs_with_origins, \
    Config, ConfigLocation, TonNetworkEnum
from wton.utils.exceptions import StorageError
...
@click.option('--global', '-g', 'config_location', flag_value=ConfigLocation.global_location.value, help='Work with a global config.')
@click.option('--local', '-l', 'config_location', default=True, flag_value=ConfigLocation.local_location.value, help='Work with the config in the current workdir. [default]')
@click.option('--file', '-f', 'config_location', help='Specific config path.', metavar='PATH')
@click.option('--network', help='Setup providers to a provided network.', type=click.Choice(TonNetworkEnum))
@click.option('--unset', is_flag=True, default=False, help='Removes [NAME] from config.')
@click.argument('name', required=False)
@click.argument('value', required=False)
@click.pass_obj
def config(shared_object: SharedObject, config_location: str, network: TonNetworkEnum, unset: bool, name: str, value: str):
    """
    Control config parameters. (check README.md for all fields info)
    """
    if config_location is None:
        config_location = ConfigLocation.local_location.value
