...
class CommandLineTool:
    ...
    def download_by_aria(self, url, dst):
        ...
        if os.path.exists(target_path):
            print('💬  ' + target_path + ' already downloaded')
    ...
    def replace_lib(self, zip_path, dst_path):
        zip_file = zipfile.ZipFile(zip_path, 'r')
        files_count = len(zip_file.namelist())
        cur_p = 0
        for file in zip_file.namelist():
            print('💬 Progress(' + str(cur_p + 1) + '/' + str(files_count) + ') Unizip file:' + file)
            zip_file.extract(file, dst_path)
            cur_p += 1
            if cur_p == files_count:
                print("💬 finished replace lib")
        zip_file.close()
        return True
    ...
    def rollback(self, branch, build_version, arch_type):
        ...
        if build_version:
            ...
            if not flag:
                ...
                return
        build_info = self.get_latest_lib_build_info(roll_back_build)
        if self.checkout_repo(build_info):
            dest_path = self.work_space_path + 'Bin/'
            release_path = self.work_space_path + 'Bin/'
            if arch_type == 'mac_x86_64':
                dest_path += ('Mac/Release' + '/x86_64_libs_' + build_info['name'] + '.zip')
                release_path += 'Mac/Release'
            else:
                dest_path += ('Mac_arm64/Release' + '/arm64_libs_' + build_info['name'] + '.zip')
                release_path += 'Mac_arm64/Release'
            self.download_by_aria(url=build_info['lib_url'], dst=dest_path)
    ...
    def get_latest_builds(self, branch, arch_type, num):
        ...
        if r.status_code == 200:
            ...
            table = PrettyTable(['Version','Created At', 'Arch_type'], title='Latest builds for ' + branch + '(' + arch_type + ')')
            res = []
            if num > 0:
                results = results[:num]
...
def cmd(conf):
    ...
    if conf is None:
        ...
        load_f.close()
    cli = CommandLineTool(api_key=conf['artifactory_api_key'], user_name=conf['artifactory_user_name'], work_space_path=conf['work_space_path'])
    if args.get('checkout-all'):
        branch_name = args.get('<branch>')
        if branch_name:
            print('Checkout all branches to ' + branch_name)
        ...
        if branch_name:
            if arch_type:
                cli.get_latest_builds(branch_name, arch_type, num)
            else:
                cli.get_latest_builds(branch_name, None, num)
...
def main():
    ...
    if not os.path.exists(conf_file_path):
        ...
        work_space_path = input('Your work space path (example:/Users/xxx/Documents/SourceCode/feature/):\n')
...
if __name__ == '__main__':
    ...
    # latest_lib_info = get_latest_lib_info()
    # print('\033[0;33m💬 Find latest lib \033[0m' + latest_lib_info['name'])
    # build_info = get_latest_lib_build_info(latest_lib_info)
    # print('💬 Find latest build repo infos:')
    # table = PrettyTable(['repo','branch','commit_hasg'], title='Repo Informations For Build ' + latest_lib_info['name'])
    # for repoinfo in build_info['repo_infos']:
    #     table.add_row([repoinfo['repo'], repoinfo['branch'], repoinfo['commit_hash']])
    # print(table)

    # release_path = work_space_path + 'Bin/Mac/Release'
    # lib_path = release_path + '/x86_64_libs_' + latest_lib_info['name'] + '.zip'
    # if artifacts_arch_type == 'mac_x86_64':
    #     release_path = work_space_path + 'Bin/Mac_arm64/Release'
    #     lib_path = release_path + '/arm64_libs_' + latest_lib_info['name'] + '.zip'
    # if checkout_repo(build_info):
    #     print('💬 Checkout all repos success!')
    #     print('💬 Start downloading lib ' + build_info['lib_url'])
    #     download_by_aria(build_info['lib_url'], lib_path, build_info['lib_size'])
    #     print('💬 Download success!')
    # print('💬 Start replacing lib ...')
    # if replace_lib(lib_path, release_path):
    #     print('💬 Replace lib finished')
    #     os.remove(lib_path)
