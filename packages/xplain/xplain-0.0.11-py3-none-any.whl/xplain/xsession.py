...
class Xsession:
    ...
    def run(self, method_call):
        ...
        :type method: json
        :return: if fails an exception will be raised, the exception message is
        the detailed stack trace from backend

        Example:
            >>> try:
                    session.run({"method": "deleteRequest",
                                  "requestName":"abcd"})
    ...
    def build_predictive_model(self, model_name, xmodel_config,
        ...
        pandas dataframes which includes independent variables,
