...
def run(dataset: DatasetSchema, location:str = None) -> Iterator[ValidationError]:
    ...
    """  # noqa: W605
    print(dataset.id, location)
    if location is not None:
        for msg in _id_unique(dataset, location):
...
def _id_unique(dataset: DatasetSchema, location: str) -> Iterator[str]:
    """Identifiers should equal the parent path to assure uniqueness.
