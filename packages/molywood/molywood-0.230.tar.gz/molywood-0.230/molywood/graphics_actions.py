...
def calc_ores(action, ovl, frames, default=False):
    ...
    if not any('relative_size' in act.overlays[ovl].keys() for act in actions if hasattr(act, 'overlays') and ovl in act.overlays.keys()):
        # except KeyError:
        if default:
...
def compose_overlay(action, master=False):
    ...
    for ovl in action.overlays.keys():
        ...
        try:
            ...
            center = False
        if not any('angle' in act.overlays[ovl].keys() for act in actions if hasattr(act, 'overlays') and ovl in act.overlays.keys()):
            angles = [0] * len(frames)
        else:
        ...
        if not any('origin' in act.overlays[ovl].keys() for act in actions if hasattr(act, 'overlays') and ovl in act.overlays.keys()):
        ...
        # except KeyError:
        if not any('alpha' in act.overlays[ovl].keys() for act in actions if hasattr(act, 'overlays') and ovl in act.overlays.keys()):
