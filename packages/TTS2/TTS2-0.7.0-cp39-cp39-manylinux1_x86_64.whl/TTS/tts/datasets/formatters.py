...
def magicdata(root_path, meta_file,ignored_speakers=None):
    txt_file = os.path.join(root_path, meta_file)
    items = []
    
    with open(txt_file, "r", encoding="utf-8") as ttf:
        for line in ttf:
            wav_name,speaker_id, text = line.rstrip("\n").split("\t")
            # ignore speakers
            if isinstance(ignored_speakers, list):
              if speaker_id in ignored_speakers:
                continue
            wav_path = os.path.join(root_path,speaker_id, wav_name)
           # if not os.path.exists(wav_path):
           #     continue
            items.append({"text": text, "audio_file": wav_path, "speaker_name": speaker_id})
    return items

def aidatatang(root_path, meta_file,ignored_speakers=None):
    txt_file = os.path.join(root_path, meta_file)
    items = []
