...
import fitz
from PyQt6.QtWidgets import QWidget, QLabel, QVBoxLayout, QToolBar, QStyle, QLineEdit
    ...
    def load_file(self, filepath: str, display=False) -> None:
        if not filepath:
            _example_pdf_path = (
                ...
                # .joinpath("../../../examples/pdf/jfesample.pdf")
                .joinpath("../FanTeX.pdf").resolve()
            )
            self.filepath = _example_pdf_path.as_posix()
            if not os.path.exists(self.filepath):
                from FanTeX.gen_FanTeX_pdf import main as gen_FanTeX

                gen_FanTeX(os.path.dirname(__file__))
        else:
