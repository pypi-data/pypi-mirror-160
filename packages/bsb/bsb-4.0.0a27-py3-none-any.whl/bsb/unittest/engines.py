...
class TestPlacementSet(RandomStorageFixture, NumpyTestCase):
    ...
    def setUp(self):
        ...
        self.cfg = Configuration.default(
            ...
            placement=dict(ch4_c25=dict(strategy="bsb.placement.strategy.FixedPositions", partitions=[], cell_types=["test_cell"]))
        )
        self.chunk_size = cs = self.cfg.network.chunk_size
        self.chunks = [Chunk((0,0,0), cs), Chunk((0,0,1), cs), Chunk((1,0,0), cs), Chunk((1,0,1), cs)]
        self.cfg.placement.ch4_c25.positions = MPI.bcast(np.vstack(
            (
    ...
    def test_load_morphologies(self):
        ...
        for i in range(2):
            ...
            self.fail(
                ...
                + ", ".join(f"'{m.name}'" for m in ms.iter_morphologies(hard_cache=True))
            )
