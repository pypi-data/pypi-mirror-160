...
* gen, to generate the classes required for Degram to run or docs
...
GENERATOR_DIR = Path('Degram_generator')
LIBRARY_DIR = Path('Degram')
...
def generate(which, action='gen'):
    from Degram_generator.parsers import\
        parse_errors, parse_methods, parse_tl, find_layer

    from Degram_generator.generators import\
...
def main(argv):
    if len(argv) >= 2 and argv[1] in ('gen', 'clean'):
        ...
        # Try importing the Degram module to assert it has no errors
        try:
            import Degram
        except:
        ...
        remove_dirs = ['__pycache__', 'build', 'dist', 'Degram.egg-info']
        ...
        for x in ('build', 'dist', 'Degram.egg-info'):
            shutil.rmtree(x, ignore_errors=True)
        ...
        with open('Degram/version.py', 'r', encoding='utf-8') as f:
        ...
        setup(
            ...
            url='https://github.com/LonamiWebs/Degram',
