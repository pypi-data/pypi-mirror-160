...
class NewMessage(EventBuilder, _custom.Message):
    ...
    to all effects as a `Message <Degram.tl.custom.message.Message>`,
    so please **refer to its documentation** to know what you can do
    with this event.

    Members:
        message (`Message <Degram.tl.custom.message.Message>`):
            This is the only difference with the received
            `Message <Degram.tl.custom.message.Message>`, and will
            return the `Degram.tl.custom.message.Message` itself,
            not the text.

            See `Message <Degram.tl.custom.message.Message>` for
