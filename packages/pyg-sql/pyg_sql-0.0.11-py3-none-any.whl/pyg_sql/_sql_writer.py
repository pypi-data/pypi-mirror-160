...
def path_to_connection(path):
    ...
    cursor = sql_table_(table = table, db = db, schema = schema, pk = 'key', non_null = dict(value = bin))
...
def sql_write(doc, root = None):
    ...
    >>> db = partial(sql_table, 
                     ...
                     pk = ['ticker', 'item'], 
                     server = 'localhost', 
                     writer = 'mssql+pyodbc://localhost/bbg?driver=ODBC+Driver+17+for+SQL+Server/dbo.bbg_data/%ticker/%item.sql', 
                     doc = True)
    >>> path = db().writer
    >>> res = path_to_connection(path)
    >>> ticker = 'CLA Comdty'
    ...
    >>> get_cell('tickers', 'bbg', server = 'localhost', ticker = ticker, item = item)
    """
