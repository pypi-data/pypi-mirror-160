...
class WebSocketSession():
    ...
    def __init__(self, context, sid, reconnect_timeout, use_dict_message):
        ...
        self.use_dict_message = use_dict_message
        self.request = None
        self.socket = None
    ...
    def to_pack_dict(self, request_id, event_id, data):
        if self.use_dict_message:
            return {'rid': request_id, 'eid': event_id, 'data':data}
        else:
            return [request_id, event_id, data]

    def pack(self, request_id, event_id, data):
        return msgpack.packb(self.to_pack_dict(request_id, event_id, data))
        ...
        if self.socket is not None and not self.socket.closed:
            try:
                if loop_response_eid == 0:
                    buf = self.pack(request_id, event_id, data)
                else:
                    buf = self.pack(request_id, loop_response_eid, self.to_pack_dict(request_id, event_id, data))
                size = len(buf)
                start = time.time()
                    ...
                    try:
                        ...
                        if size < 1024**2 * 4:
                            ...
                            for buf in iter(lambda: bio.read(1024*1024 * 2), b''):
                                buf = self.pack(request_id, EID_DIVIDED_RESPONSES_APPEND, buf)
                                async with self.send_lock:
                                    await asyncio.shield(self.socket.send_bytes(buf))
                                log.debug("SEND_BYTES_DIV|SENTB|TIME=%s", time.time()-start)
                                await asyncio.shield(self.socket.drain())
                            await asyncio.shield(self.socket.send_bytes(self.pack(request_id, EID_DIVIDED_RESPONSES_END, None)))
                            await asyncio.shield(self.socket.drain())
...
class WebSocketServer(object):
    ...
    async def websocket_handler(self, request):
        ...
        if sid in self.context.session_store:
            ...
            session = WebSocketSession(self.context, sid, self.conf.reconnect_timeout, param.get('ua') == 'bob')
    ...
    def unpack(self, data, use_dict_message):
        if use_dict_message:
            kv = msgpack.unpackb(data)
            return kv['rid'], kv['eid'], kv['data']
        else:
            return msgpack.unpackb(data)

    async def _msgpack_msg_handler(self, socket_session, msg, received):
        request_id, event_id, data = self.unpack(msg.data, socket_session.use_dict_message)
