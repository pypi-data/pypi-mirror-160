...
# Make a node name
def make_name(node_index, set, depth, score, unaccounted_courses, all_courses_allocated, UB, pf):
    node_name = str(node_index) + ": "
    for course in set:
        node_name += course.name + " " + "|rem.: " + str(course.remaining_slots)
    node_name += "|depth: " + str(depth) + "| pf: " + str(pf) #"|score: " + str(score) + "|UB: " + str(UB)
    if depth > 26:
        node_name+="|"
...
class Node_Data:
    ...
    # constructor
    def __init__(self, depth, course_set, courses_to_schedule, courses_partially_scheduled, UB, score, score_avg, num_courses_scheduled, complete):
        self.depth = depth
        ...
        self.num_courses_scheduled = num_courses_scheduled
