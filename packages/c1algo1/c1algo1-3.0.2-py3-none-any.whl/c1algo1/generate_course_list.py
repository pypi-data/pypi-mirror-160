        ...
        for course in courses_and_profs['schedule'][current_semester]:
            ...
            if len(course['sections']) > 0:
                ...
                if num_assigned_profs == num_sections and num_assigned_timeslots == num_sections:
                    ...
                    # Add course to shceduled_courses list before skipping to avoid scheduling conflicts with this course
                    #for section in course['sections']:
                    if True:
                        course_name = course['course']['code']
                        section_index = 0
                        while( section_index < len(course['sections']) ):
                            for timeSlot in course['sections'][section_index]['timeSlots']:
                                section_index += 1


                                start_time = timeSlot['timeRange'][0]
                                end_time = timeSlot['timeRange'][1]
                                ...
                                while( TWF_index < len(timeslots) ):
                                    ...
                                    if(timeslots[TWF_index]['start_time'] == start_time): # If start time, set assigned_slot to True
                                        ...
                                        if day == "MONDAY" or day == "THURSDAY": # set MTh_index if scheduled on MTh
                                    ...
                                    if assigned_to_slot: # If assigned slot, add course to list of scheduled courses
                                        ...
                                        if day == "MONDAY" or day == "THURSDAY": # Check if course should be added at MTh index
                                            #if not MTh_course_scheduled:
                                            course_scheduled = False
                                            for sc in scheduled_courses[current_semester][MTh_index]:
                                                if sc.name == scheduled_course.name:
                ...
                if consecutive_offerings > 3:
                    ...
                    while( TWF_index < len(timeslots) ):
                        ...
                        if assigned_to_slot: # If assigned slot, add course to list of scheduled courses
                            ...
                            # make course object here
                            semester = get_onstream_semester(course_name)
                            scheduled_course = Course(course_name, semester, prof, [], current_semester)

                            if day_spread == "MTh": # Check if course should be added at MTh index
                                 # Check if conflict with static course or already scheduled historical course
                                conflict = False
                                for sc in scheduled_courses[current_semester][MTh_index]:
                                    if sc.semester == scheduled_course.semester:
                                        conflict = True
                                if conflict:
                                    if scheduled_course in scheduled_courses[current_semester][MTh_index-1]:
                                        scheduled_courses[current_semester][MTh_index-1].remove(scheduled_course)
                                    if scheduled_course in scheduled_courses[current_semester][MTh_index+1]:
                                        scheduled_courses[current_semester][MTh_index+1].remove(scheduled_course)
                                    scheduling_historically = False
                                    break
                                 ...
                                 # Check if conflict with static course or already scheduled historical course
                                conflict = False
                                for sc in scheduled_courses[current_semester][TWF_index]:
                                    if sc.semester == scheduled_course.semester:
                                        conflict = True
                                if conflict:
                                    if scheduled_course in scheduled_courses[current_semester][TWF_index-1]:
                                        scheduled_courses[current_semester][TWF_index-1].remove(scheduled_course)
                                    if scheduled_course in scheduled_courses[current_semester][MTh_index-1]:
                                        scheduled_courses[current_semester][MTh_index-1].remove(scheduled_course)
                                    if scheduled_course in scheduled_courses[current_semester][MTh_index-2]:
                                        scheduled_courses[current_semester][MTh_index-2].remove(scheduled_course)
                                    scheduling_historically = False
                                    break
                                else:
                                    scheduled_courses[current_semester][MTh_index].append(scheduled_course)
            ...
            if not scheduling_historically:
                time_scores = [0] * 54
...
def get_ppw():
    ppw = {
        ...
        "CSC361": {
            "CourseID": "CSC361",
            "CourseName": "Cmputer Communications and Networks",
            "OnstreamSemester": "3A",
            "OfferedIn": "F,Sp",
            "Capacity": 120
        },
        ...
        "CSC460": {
            "CourseID": "CSC460",
            "CourseName": "Design and Analysis of Real-time Systems",
            "OnstreamSemester": "4B",
            "OfferedIn": "Sp",
            "Capacity": 120
        },
