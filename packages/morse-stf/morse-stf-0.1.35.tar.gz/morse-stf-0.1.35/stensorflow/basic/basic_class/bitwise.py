            ...
            if other.owner == self.owner:
                ...
                with tf.device(StfConfig.RS[0]):
                    if prf_flag:
                        ...
                        seed_other = get_seed()
                        seed_z = get_seed()
                        self_adjoint = self.random_uniform_adjoint(seed_self)
                        other_adjoint = other.random_uniform_adjoint(seed_other)
                        z = self_adjoint * other_adjoint
                        zL = z.random_uniform_adjoint(seed_z)
                    else:
            ...
            if self.ownerL == other.ownerL and self.ownerR == other.ownerR:
                with tf.device(StfConfig.RS[0]):
                    if prf_flag:
                        ...
                        xL_adjoint = self.xL.random_uniform_adjoint(seed_xL)
                        xR_adjoint = self.xR.random_uniform_adjoint(seed_xR)
                        yL_adjoint = other.xL.random_uniform_adjoint(seed_yL)
                        yR_adjoint = other.xR.random_uniform_adjoint(seed_yR)
                        z_adjoint = (xL_adjoint + xR_adjoint) * (yL_adjoint + yR_adjoint)
                        zL_adjoint = z_adjoint.random_uniform_adjoint(seed_z)
                    else:
