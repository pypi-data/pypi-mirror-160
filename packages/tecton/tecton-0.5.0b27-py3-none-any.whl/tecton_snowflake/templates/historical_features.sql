    ...
    {%- for item in feature_set_items -%}
        ...
        {%- if item.aggregation -%}
            {%- for feature in item.aggregation.features %}
                {{- final_select_name(item.namespace + separator + feature.output_feature_name, feature.function|snowflake_function == "count", not (loop.last and last_item)) | indent }}
            {%- endfor -%}
        {%- else %}
            {%- for feature in item.features %}
                {{- final_select_name(item.namespace + separator + feature, false, not (loop.last and last_item)) | indent }}
            {%- endfor -%}
        {%- endif -%}
    {%- endfor %}
