...
class BacnetClient:
                ...
                if key not in self.bacnet_objects.keys():
                    ...
                    if object_class is not None:
                        new_object = object_class(
                            ...
                            presentValue=self._convert_value_to_set(object.get('point_value', ''), get_datatype(object_type, 'presentValue'), None),
    ...
    def refresh_objects(self, objects: List[IOTSimulateObject]):
        for object in objects:
            object_type = object.get('point_type')
            instance_number = object.get('point_address')
            point_value = object.get('point_value')
            if object_type is not None and instance_number is not None and point_value is not None:
                key = f"{object_type}_{instance_number}"
                if key in self.bacnet_objects.keys():
                    self.bacnet_objects[key][0].presentValue = self._convert_value_to_set(object.get('point_value', ''), get_datatype(object_type, 'presentValue'), None)
...
class IOTBacnet(IOTDriver):
    ...
    def template(cls, mode: int, type: str, lan: str) -> List[Dict[str, Any]]:
        ...
        if type == 'point':
            ...
            templates.append({'name': '超时(s)' if lan == 'ch' else 'Timeout(s)', 'required': True, 'code': 'time_out', 'type': 'int', 'default': 5, 'enum': [], 'tip': ''})
        return templates
    ...
    def _get_client(self):
        if self.client is None:
            self.client = BacnetClient(self, **self.configs)
        return self.client
    ...
    def _get_server(self):
        if self.server is None:
            self.server = BacnetClient(self, **self.configs)
        return self.server
