...
class IOTOPCUA(IOTDriver):
    ...
    def template(cls, mode: int, type: str, lan: str) -> List[Dict[str, Any]]:
        ...
        if type == 'point':
            ...
            templates.append({'required': True, 'name': 'OPC服务地址' if lan == 'ch' else 'Endpoint', 'code': 'endpoint', 'type': 'string', 'default': 'opc.tcp://192.168.1.182:9503/freeopcua/server/', 'enum': [], 'tip': ''})
            if mode == 0:
                templates.append({'required': True, 'name': 'OPC超时' if lan == 'ch' else 'TimeOut', 'code': 'timeout', 'type': 'int', 'default': 4, 'enum': [], 'tip': ''})
                ...
                templates.append({'required': False, 'name': '通道超时(s)' if lan == 'ch' else 'Channel Timeout(s)', 'code': 'channel_timeout', 'type': 'int', 'default': 10000, 'enum': [], 'tip': ''})
                templates.append({'required': False, 'name': '会话超时(s)' if lan == 'ch' else 'Session Timeout(s)', 'code': 'session_timeout', 'type': 'int', 'default': 10000, 'enum': [], 'tip': ''})
            else:
            ...
            templates.append({'required': True, 'name': '更新周期(ms)' if lan == 'ch' else 'Interval(ms)', 'code': 'interval', 'type': 'int', 'default': 60000, 'enum': [], 'tip': ''})
        return templates
    ...
    def read(self, **kwargs):
        ...
        for name in names:
            ...
            if point:
                node_id = point.get('point_node_id')
                if node_id is not None:
                    value = self.sub_nodes.get(self._get_client().get_node(node_id).nodeid.to_string())
        ...
        for object in objects:
            ...
            if node_id not in self.server_nodes.keys():
                ...
                if len(node_path) > 0:
                    ...
                    self.server_nodes[node_id] = folder.add_variable(node_id, paths[-1], object.get('point_value'))
    ...
    def _refresh_objects(self, objects: List[IOTSimulateObject]):
        for object in objects:
            ...
            if node_id in self.server_nodes.keys():
                self.server_nodes[node_id].set_value(object.get('point_value'))
                #ua.DataValue(ua.Variant(20, ua.VariantType.Int16), ua.StatusCode(ua.StatusCodes.Bad))
