...
class Handler(metaclass=abc.ABCMeta):
        ...
        def wrapper(body: Any, *, metadata: Optional[dict] = None) -> Any:
            ...
            with sentry_sdk.configure_scope() as scope:
                ...
                try:
                    ...
                    logger.exception(
                        f'Unhandled exception "{exception}" was suppressed in '
                        f'"{function}".',
                        extra={
                            "dry_run": dry_run,
                            "function": function,
    ...
    def _configure_sentry_scope(
        ...
        *,
        ...
        qualname = getattr(
            function, "__qualname__", function.__class__.__qualname__
        )
        scope.set_tag("clink_handler", f"{function.__module__}.{qualname}")
        scope.set_tag("dry_run", dry_run)
...
class EventHandler(Handler, metaclass=abc.ABCMeta):
    ...
    def _configure_sentry_scope(
        ...
        *,
        ...
        scope.set_context("event", event.to_dict())
        scope.set_tag("event.source", str(event.source))
        scope.set_tag("event.type", str(event.type))
        scope.set_tag("trace_id", str(event.trace_parent.trace_id))
        if event.subject_urn:
            utils.set_scope_tags(
                scope,
                tags={
                    "event.subject": str(event.subject_urn),
                    "event.subject.partition": str(
                        event.subject_urn.partition
                    ),
                    "event.subject.service": str(event.subject_urn.service),
                    "event.subject.region": str(event.subject_urn.region),
                    "event.subject.account": str(event.subject_urn.account),
                    "event.subject.resource.type": str(
                        event.subject_urn.resource.type
                    ),
                    "event.subject.resource.id": str(
