...
'''
    ...
    def stats(self,keep='',niter=10,disp=True):
        ...
        for n in range(niter):                      
            ...
            print('* Mu: the averaged subsample mean;\n* SE: the standard error of Mu in %;')
            print('* Std: the averaged subsample standard deviation;\n* Min: the minimum of the subsample minimum;')
            print('* Med: the median of the subsample median;\n* Max: the maximum of the subsample maximum;')
            print('* Skew: the averaged sample skewness;\n* Kurt: the averaged non-centralized kurtosis.')
        ...
        for n in range(niter):
            ...
            for j in range(ncov):
                ...
                if tv == True:
                    ...
                    '''If the scale levels have not been specified'''
                    if mylist[j] not in self.pm.scale_level_record:
                        for k in range(max_output_number):
                            print(clevels[k], '=', ('%.2f'%prob[k]), '%', end='|') 
                        if len(clevels) > 100:
                            print('*** Note: only top 100 levels displayed...')  
                    else:
                        _scale_levels = copy.deepcopy(self.pm.scale_level_record[mylist[j]])
                        n_scale_levels = len(_scale_levels)
                        _prob_in_scale_levels = .0
                        for k in range(max_output_number):
                            _scale_str = str(clevels[k])
                            if _scale_str in _scale_levels:
                                print(clevels[k], '=', ('%.2f'%prob[k]), '%', end='|')
                                _prob_in_scale_levels += prob[k]
                                _scale_levels.remove(_scale_str)
                        for _k in range(len(_scale_levels)):
                            print(_scale_levels[_k], '= 0.00 %', end='|')
                        print('Others = %.2f %%' % (100-_prob_in_scale_levels), end='|')
                        if n_scale_levels > 100:
                            print('*** Note: only top 100 levels displayed...')
