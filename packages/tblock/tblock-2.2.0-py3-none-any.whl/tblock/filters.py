...
# Local modules
from .const import FilterPermissions, WARNS, RulePolicy
...
class Filter:
    ...
    def __init__(self, filter_id: str, quiet: bool = False, custom_source: str = None) -> None:
        ...
        self.rules_count = data[7] if self.exists else None
    ...
    if subscribing_only and not_subscribing_only or from_repo_only and custom_only:
        ...
        with sqlite3.connect(Path.DATABASE) as conn:
            if subscribing_only and from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1 AND on_repo=1;")
            elif subscribing_only and custom_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1 AND on_repo=0;")
            elif subscribing_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=1;")
            elif not_subscribing_only and from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0 AND on_repo=1;")
            elif not_subscribing_only and custom_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0 AND on_repo=0;")
            elif not_subscribing_only:
                data = conn.execute("SELECT id FROM filters WHERE subscribing=0;")
            elif custom_only:
                data = conn.execute("SELECT id FROM filters WHERE on_repo=0;")
            elif from_repo_only:
                data = conn.execute("SELECT id FROM filters WHERE on_repo=1;")
            else:
                data = conn.execute("SELECT id FROM filters;")
        output_list = []
        for row in data:
