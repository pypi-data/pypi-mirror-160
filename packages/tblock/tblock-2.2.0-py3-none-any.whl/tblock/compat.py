...
def setup_new_database(quiet: bool = False) -> None:
    ...
    with sqlite3.connect(Path.DATABASE) as conn:
        ...
        conn.cursor().execute('''CREATE TABLE IF NOT EXISTS "cache" (
...
def update_database_from_1_3_2(quiet: bool = False) -> None:
    ...
    """
    if not quiet:
        print(f'{Font.BOLD}==> Updating database from v1.3.2 to v{VERSION}{Font.DEFAULT}')
    log_message(f"[core]    INFO:  updating sqlite database from v1.3.2 to v{VERSION} under: {Path.DATABASE}")
...
def update_database_from_1_2_0(quiet: bool = False) -> None:
    ...
    """
    if not quiet:
        print(f'{Font.BOLD}==> Updating database from v1.2.0 to v{VERSION}{Font.DEFAULT}')
        log_message(f"[core]    INFO:  updating sqlite database from v1.2.0 to v{VERSION} under: {Path.DATABASE}")
    with sqlite3.connect(Path.DATABASE) as conn:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    db_version = conn.cursor().execute(
                        'SELECT value FROM system WHERE variable="db_version";').fetchone()[0]
                except sqlite3.OperationalError:
                    log_message(
                        f"[core]    INFO:  an unknown error occurred while upgrading the database to version {VERSION}")
                else:
                    if db_version < VERSION:
                        print(f'{Font.BOLD}==> Updating database from v{db_version} to v{VERSION}{Font.DEFAULT}')
                        log_message(f"[core]    INFO:  updating sqlite database from "
                                    f"v{db_version} to v{VERSION} under: {Path.DATABASE}")
                        setup_new_database(quiet=quiet)
                        conn.cursor().execute('UPDATE system SET value=? WHERE variable="db_version";', (VERSION,))
                        conn.commit()
                        conn.close()
            else:
