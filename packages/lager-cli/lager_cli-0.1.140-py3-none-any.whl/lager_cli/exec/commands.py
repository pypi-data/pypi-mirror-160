...
def _run_command_host(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty, user, group, macaddr, hostname):
    """
...
def _run_command(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty, user, group, macaddr, hostname):
    ...
    if ci_env in (CIEnvironment.HOST, CIEnvironment.JENKINS):
        return _run_command_host(section, path, cmd_to_run, mount, extra_args, debug, interactive, tty, user, group, macaddr, hostname)
...
def exec_(ctx, cmd_name, extra_args, command, save_as, warn, mount, interactive, tty, user, group):
    ...
    returncode = _run_command(section, path, 'set -e;' + cmd_to_run, mount, extra_args, ctx.obj.debug, interactive, tty, user, group, macaddr, hostname)
    ctx.exit(returncode)
