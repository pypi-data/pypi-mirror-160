...
@click.argument('instru', type=click.Choice(['ska_low', 'nenufar', 'lofar_hba', 'nenufar_full', 'a12_hba'], case_sensitive=False))
...
def simu_uv(instru, z, out_filename, umin, umax, dec_deg, fov, bandwidth, df, total_time, int_time, 
    ...
    telescop = obssimu.TelescopeSimu.from_name(instru, freqs, dec_deg, 0, total_time, umin,
    ...
    print(f'Baseline range: {telescop.umin} - {telescop.umax} lambda (discarding intra-baselines: {telescop.remove_intra_baselines})')
...
def simu_noise_ps(uv_simu_h5, total_time_hour, out_filename, kmin, kmax, nks, 
                  filter_kpar_min, filter_wedge_theta, sefd, min_weight):
    ...
    telescop = uv_simu.telescope_simu
