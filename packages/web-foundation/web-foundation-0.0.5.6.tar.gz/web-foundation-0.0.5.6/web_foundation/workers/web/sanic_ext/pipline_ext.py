...
ProtectService = TypeVar("ProtectService", bound=object)


class Protector(Generic[ProtectService]):
    ...
    @classmethod
    async def protect(cls, context: InputContext, cls_self: ProtectService) -> ProtectIdentity | None:
        return None
...
def set_to_pipline(uri: str,
    ...
    def called_method(target: MethodType):
        ...
        async def f(*args, **kwargs):
            ...
            prot_identity = await (protector(n_ctx).protect(incoming, class_of_method(target)))
