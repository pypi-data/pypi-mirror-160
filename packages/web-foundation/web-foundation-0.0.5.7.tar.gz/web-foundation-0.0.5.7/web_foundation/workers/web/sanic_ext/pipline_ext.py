...
from web_foundation.workers.web.sanic_ext.structs import InputContext, ProtectIdentity, HTTPMethod, DtoType
...
class RequestParser:
    ...
    def income_(self, request: Request, **kwargs) -> InputContext:
        incoming_context = InputContext[self.struct](r_args=SimpleNamespace(), r_kwargs={},
                                                     named_ctx=request.app.ctx.named_ctx,
...
ProtectionService = TypeVar("ProtectionService")


class Protector(Generic[ProtectionService]):
    _from_service: Type[ProtectionService]
    from_service: ProtectionService

    def __init__(self, named_ctx: NamedContext):
        self.from_service = named_ctx.get_obj(self._from_service)

    async def protect(self, context: InputContext) -> ProtectIdentity | None:
        return None
...
def set_to_pipline(uri: str,
    ...
    def called_method(target: MethodType):
        ...
        async def f(*args, **kwargs):
            ...
            prot_identity = await (protector(n_ctx).protect(incoming))
