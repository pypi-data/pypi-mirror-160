...
CheckNoneType = TypeVar("CheckNoneType")
...
def avoid_none(instance: CheckNoneType | None) -> CheckNoneType:
    if not instance:
        raise Exception("Need instance not found")
    else:
        return instance
...
class AccessDMW(DatabaseMiddleware):
    ...
    @staticmethod
    async def create(model: GenericDbType, protection: AccessProtectIdentity, dto: PdModel | None,
                     **kwargs) -> GenericDbType:
        entity_kwargs = {field: value for field, value in avoid_none(dto).dict().items() if value is not None}
        try:
    ...
    @staticmethod
    async def read(model: GenericDbType, protection: AccessProtectIdentity, fetch_fields: List[str] | None = None,
    ...
    @staticmethod
    async def update(model: GenericDbType, protection: AccessProtectIdentity, dto: PdModel | None,
                     **kwargs) -> GenericDbType:
        entity = await AccessDMW.get_entity(model, avoid_none(protection.entity_id))
        entity_kwargs = {field: value for field, value in avoid_none(dto).dict().items() if value is not None}
    ...
    @staticmethod
    async def delete(model: GenericDbType, protection: AccessProtectIdentity, **kwargs) -> GenericDbType:
        entity = await AccessDMW.get_entity(model, avoid_none(protection.entity_id))
        await entity.delete()
        return entity
