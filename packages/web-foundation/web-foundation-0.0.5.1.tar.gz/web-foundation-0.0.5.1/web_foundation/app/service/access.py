...
from web_foundation.app.infrastructure.database.middleware import AccessDMW, AccessProtectIdentity  # type: ignore
        ...
        match inc.request.method:
            case HTTPMethod.GET.value:
                ...
                if prot.entity_id is None:
                    ...
                    return res
                retrieved = await access_middleware.read(model, prot, **kwargs)
                if retrieved:
            ...
            case HTTPMethod.PATCH.value:
                assert inc.dto
                retrieved = await access_middleware.update(model, prot, inc.dto, **inc.r_kwargs)
                return json(await retrieved.values_dict())

            case HTTPMethod.DELETE.value:
                retrieved = await access_middleware.delete(model, prot, **inc.r_kwargs)
                return json(await retrieved.values_dict())
