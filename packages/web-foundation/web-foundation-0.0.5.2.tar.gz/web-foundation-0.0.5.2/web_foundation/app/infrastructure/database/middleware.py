...
class AccessDMW(DatabaseMiddleware):
    ...
    @staticmethod
    async def update(model: GenericDbType, protection: AccessProtectIdentity, dto: PdModel,
                     **kwargs) -> GenericDbType:
        assert protection.entity_id is not None
        entity = await AccessDMW.get_entity(model, protection.entity_id)
        entity_kwargs = {field: value for field, value in dto.dict().items() if value is not None}
