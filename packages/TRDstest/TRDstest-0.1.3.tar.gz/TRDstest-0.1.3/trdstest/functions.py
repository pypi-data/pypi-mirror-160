import numpy as np
from trdstest.base import unfold, fold
from ops import Z_neq, tenmat_sb
...
def n_mode_product(T, M, n):
    ...
    if M.ndim == 1:
        if len(res_shape) > 1:
            ...
            res_shape[1] = 1
        product = fold(product_, n, res_shape).squeeze()
    else:
        product = fold(product_, n, res_shape)
    return product
...
def n_mode_inner_product(T1, T2, n):
    ...
    if list(T1.shape)[-n:] != list(T2.shape)[:n]:
        raise ValueError('Shapes of inner product along modes incorrect')
