...
def ciselnikJSON(df, file_name):
    ...
    json_file_name = '-'.join(file_name.lower().split(' ')) + '.json'  # název spojený pomlčkami, malá písmena
...
def ciselnikCSV(df, file_name):
    ...
    csv_file_name = '-'.join(file_name.lower().split(' ')) + '.csv'  # název spojený pomlčkami, malá písmena
