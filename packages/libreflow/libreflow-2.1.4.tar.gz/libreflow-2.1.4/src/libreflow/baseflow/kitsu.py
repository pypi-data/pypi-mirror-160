...
# class KitsuShot(Entity):

#     kitsu_shot_names = Property()


# class KitsuShots(EntityCollection):

#     @classmethod
#     def mapped_type(cls):
#         return KitsuShot
    
#     def add_shot(self, shot_oid):
#         self.get_entity_store().get_collection(self.collection_name()).insert_one(
#             {'name': self._oid_to_mapped_name(shot_oid)},
#             {'kitsu_shot_names': []}
#         )
    
#     def _oid_to_mapped_name(self, oid):
#         return oid.replace('/', '_')[1:]


class AbstractKitsuShot:

    def set_task_status(self, task_name, task_status):
        raise NotImplementedError((
            'Must set the status of the given task '
            'of the Kitsu shot entity corresponding '
            'to this shot object.'
        ))
