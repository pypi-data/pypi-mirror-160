    ...
    if Params.CALC_CONDUCT and Planet.Do.VALID:
        ...
        Planet.Magnetic.Texc_hr, Planet.Magnetic.omegaExc_radps, Planet.Magnetic.Benm_nT, Planet.Magnetic.B0_nT \
            = GetBexc(Planet.name, Planet.Magnetic.SCera, Planet.Magnetic.extModel, Params.Induct.excSelectionCalc,
                      nprmMax=Planet.Magnetic.nprmMax, pMax=Planet.Magnetic.pMax, SUM_NEAR=Params.Induct.SUM_NEAR,
                      USE_NAMED_EXC=Params.Induct.USE_NAMED_EXC, minBe_nT=Params.Induct.minBe_nT)
...
def GetBexc(bodyname, era, model, excSelection, nprmMax=1, pMax=0, SUM_NEAR=True, USE_NAMED_EXC=True, minBe_nT=1.0):
    """ Read in magnetic excitation information, including oscillation
        ...
        Args:
            ...
            SUM_NEAR = True (bool): Whether to sum oscillations that are within 0.01 h of
                named periods when multiple satisfy this criterion
            USE_NAMED_EXC = True (bool): Whether to use the periods listed in Moments.py or just
                those exceeding minBe_nT.
            minBe_nT = 1.0 (float): Minimum excitation moment to include when none are
                specified in Excitations(PlanetName).
        Returns:
    ...
    if BeLabel in EOSlist.loaded.keys():
        ...
        if os.path.isfile(os.path.join(fPath, f'{fNames[0]}.txt')):
            ...
            if USE_NAMED_EXC and len(BeList.keys()) != 0:
                ...
                for oscillation, included in excSelection.items():
                    if included and BeList[oscillation] is not None:
                        if not SUM_NEAR:
                            ...
                            iMatch = np.round(inpTexc_hr,2) == BeList[oscillation]
                            Texc_hr[iPeak] = np.mean(inpTexc_hr[iMatch])
                            Benm_nT[iPeak, ...] = np.sum(inpBenm_nT[iMatch, ...], axis=0)
                        iPeak += 1
            else:
                Bex, Bey, Bez = Benm2absBexyz(inpBenm_nT)
                BeMag = np.sqrt(Bex**2 + Bey**2 + Bez**2)
                Texc_hr = np.sort([T for T,Be in zip(inpTexc_hr, BeMag) if Be >= minBe_nT])
                nPeaks = np.size(Texc_hr)
                Benm_nT = np.zeros((nPeaks, 2, nprmMax+pMax+1, nprmMax+pMax+1), dtype=np.complex_)
