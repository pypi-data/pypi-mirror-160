...
class NewebpayResume:
    ...
    def parse_payload(self, checkout: Checkout, **kwargs: Any) -> CheckoutModel:
        ...
        counter = get_collection("counter").find_one_and_update(
            {"collection": "checkout", "order_number": checkout.order_number},
            {
                "$inc": {"idx": 1},
                "$setOnInsert": {
                    "collection": "checkout",
        ...
        return CheckoutModel(
            ...
            MerchantOrderNo=f"{checkout.order_number}_{counter['idx'] +1}",
...
class EcpayResume:
    ...
    def parse_payload(self, checkout: Checkout, **kwargs: Any) -> EcCheckoutModel:
        ...
        return EcCheckoutModel(
            ...
            ItemName=item_name,
            MerchantTradeNo=f"{checkout.order_number}_{counter['idx'] +1}",
            MerchantTradeDate=now.strftime("%Y/%m/%d %H:%M:%S"),
            ReturnURL=self.return_url,
            **type_map,
            **kwargs,
