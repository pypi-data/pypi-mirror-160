...
class JwtToken(BaseModel):
    ...
    institutionCentralId: ObjectIdStr = None
    institutionBranchId: ObjectIdStr = None
...
async def get_current_user(security_scopes: SecurityScopes, token: str = Depends(oauth2_scheme)):
    try:
        ...
        data_token.institutionCentralId = payload.get("institutionCentralId")
        data_token.institutionBranchId = payload.get("institutionBranchId")
...
async def create_refresh_token(response, token):
    try:
        ...
        if datetime.utcfromtimestamp(payload.get("exp")) > datetime.utcnow():
            ...
            access_token = await create_access_token(data_token, config.ACCESS_TOKEN_EXPIRE_MINUTES)
            # print(access_token)
