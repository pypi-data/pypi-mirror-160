...
class CloudController(BaseController):
    ...
    def experimental_setup_secrets(
        self, cloud_name: Optional[str], cloud_id: Optional[str], yes: bool = False
    ):
        """
        ...
        if not feature_flag_on:
            raise ClickException(
                "Secrets can only be set up if the wandb-integration-prototype feature "
                "flag is serving True."
            )
        ...
        if cloud.provider == "AWS":
            return self._experimental_grant_secrets_access_aws(cloud, yes)
    ...
    def _experimental_grant_secrets_access_aws(self, cloud: Cloud, yes: bool) -> None:
        ...
        policy_name = f"anyscale-secrets-readonly-{cloud.id}"

        role.Policy(name=policy_name).put(
            PolicyDocument=json.dumps(ANYSCALE_SSM_READONLY_ACCESS_POLICY_DOCUMENT),
        )
        self.log.info(
