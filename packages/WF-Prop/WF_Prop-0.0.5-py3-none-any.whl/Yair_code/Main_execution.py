...
def Gui():
    ...
    layout = [[sg.Menu(menu_def, tearoff=False)],
              ...
              [sg.Frame('Convergence Tests',layout=[[sg.Checkbox('Spacial grid density', size=(40,1),key='-density-')], [sg.Checkbox('System size',  size=(40,1),key='-sys_size-')],[sg.Checkbox('Time steps', size=(40,1),key='-dt-')]])],
              ...
              [sg.Text(
                  'Please input here the Initial number of spacial partitions. The number of dx. This should be an integer')],
    ...
    # ------------------------------------------------------------------------------------------
     ...
     # ------------------------------------------------------------------------------------------
        ...
        # first step is to get information about the spacial partition - the Nt's number.
        ...
        if not overall_values['-density-'] and overall_values['-Nt-'] == "" and (not(event == sg.WIN_CLOSED or event == 'Exit' or event == 'Cancel') or \
            ...
            layout_temp = [[sg.Menu(menu_def, tearoff=True)],
                           [sg.Text(
                               'Please supply the number of spacial partition you wish your system to have. The # of dx`s ',
                               size=(50, 1))],
        ...
        if overall_values['-density-'] and (not(event == sg.WIN_CLOSED or event == 'Exit' or event == 'Cancel') or \
            ...
            layout_temp = [[sg.Menu(menu_def, tearoff=True)],
                           [sg.Text(
                               ...
                               size=(100, 1))],
                           [sg.In(default_text=100, size=(20, 1), key='-conv_dens_iter-')],
                           [sg.Submit(), sg.Cancel()]]
            ...
            if overall_values['-ex_loc_mul-']:
                if overall_values['-interface-']:
                    ...
                    stg2 = Stage_2(Locp, grid_density, psi0_dic, Nt=overall_values['-Nt-'], dt=overall_values['-initial_dt-'],
                                   To_plot=True, Has_interface=False,multi_left = left_multi, multi_right = right_multi)
                    ...
                    stg2.elongated_locpot_vec = stg2.update_converged_spacial_grid(vecor_to_update=stg2.elongated_locpot_vec,
    ...
    # Transmission Coefficient
        ...
        if overall_values['-Trans_coeff-'] and (not(event == sg.WIN_CLOSED or event == 'Exit' or event == 'Cancel') or \
            ...
            if overall_values['-Range-']:
                try:
                    results_Trans = transmision_coeff(stg2.psi0,stg2.Nt,stg2.dt,stg2.current_locpot_vec[:,1],stg2.current_locpot_vec[:,0],kk,overall_values['-ref_pos-'],path_to_save=overall_values['-sys_size_save_path-'])
                    path_to_save = overall_values['-sys_size_save_path-']
                except KeyError:
                    try:
