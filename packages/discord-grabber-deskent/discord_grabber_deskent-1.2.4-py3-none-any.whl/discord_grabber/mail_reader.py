import re
import imaplib

import email
import datetime
import base64

from myloguru.my_loguru import get_logger


class MailReader:
    def __init__(self, email: str, password: str, server: str, logger=None, log_level: int = 20):
        self.login: str = email
        self.password: str = password
        self.logger = logger or get_logger(log_level)
        self.data: list = []
        self.imap: 'imaplib.IMAP4_SSL' = imaplib.IMAP4_SSL(server)

    @staticmethod
    def _b64_pad_and_decode(b: bytes):
        """Decode unpadded base64 data"""

        b += (-len(b) % 4) * '='  # base64 padding (if adds '===', no valid padding anyway)
        return base64.b64decode(b, altchars='+,', validate=True).decode('utf-16-be')

    @staticmethod
    def _imap_utf7_decode(string: str):
        """Decode a string encoded according to RFC2060 aka IMAP UTF7.

        Minimal validation of input, only works with trusted data"""
        lst = string.split('&')
        out = lst[0]
        for e in lst[1:]:
            u, a = e.split('-', 1)  # u: utf16 between & and 1st -, a: ASCII chars folowing it
            if u == '':
                out += '&'
            else:
                out += MailReader._b64_pad_and_decode(u)
            out += a
        return out

    @staticmethod
    def _imap_utf7_encode(string: str) -> str:
        """"Encode a string into RFC2060 aka IMAP UTF7"""

        string = string.replace('&', '&-')
        unipart = out = ''
        for c in string:
            if 0x20 <= ord(c) <= 0x7f:
                if unipart != '':
                    out += '&' + base64.b64encode(unipart.encode('utf-16-be')).decode('ascii').rstrip('=') + '-'
                    unipart = ''
                out += c
            else:
                unipart += c
        if unipart != '':
            out += '&' + base64.b64encode(unipart.encode('utf-16-be')).decode('ascii').rstrip('=') + '-'
        return out

    def reader_login(self):
        self.imap.login(self.login, self.password)

    def reader_logout(self):
        self.imap.logout()

    def read_folder(self, folder):
        self.imap.select(self._imap_utf7_encode(folder))
        result, data = self.imap.uid('search', '', 'ALL')
        if data:
            self.data = data[0].split()[::-1]

    def get_link_from_last_email(self):
        self.logger.debug('read folder...')
        self.read_folder('inbox')
        new_count = 0
        for uid in self.data:
            self.logger.info(f'Starting work for uid: {uid} ')
            try:

                uid = int(uid.decode('utf-8'))
                new_count += 1

                result, msg = self.imap.uid('fetch', str(uid), '(RFC822)')

                if not msg[0]:
                    self.logger.debug(f' message {msg}')
                    continue

                mail = email.message_from_bytes(msg[0][1])

                dt: 'datetime' = datetime.datetime.strptime(' '.join(
                    mail['Date'].split(' ')[1:4]), '%d %b %Y')
                subj = mail['Subject']
                self.logger.debug(f'{mail["From"]}, {mail["Content-Type"]}')
                self.logger.debug(f'mail to {mail["To"]} data {dt}, subject {subj}')
                body = ''
                if mail.is_multipart():
                    for payload in mail.get_payload():
                        if payload.get_content_maintype() == 'text':
                            body = payload.get_payload(decode=True).decode('utf-8', 'ignore')

                else:
                    body = mail.get_payload(decode=True).decode('utf-8', 'ignore')

                search = re.search(r'<a href="(https://click\.discord\.com.[^"]*?)".[^>]*?>([^<:]*?)</a>', body)
                result: str = search.group(1)
                self.logger.debug(f'Result: {result}')

                return result
            except Exception as err:
                self.logger.error(err)

    def start(self) -> str:
        self.reader_login()
        link: str = self.get_link_from_last_email()
        self.reader_logout()

        return link
