...
class Transaction:
    ...
    def __init__(
        ...
        if fetch:
            ...
            try:
                ...
                user_agent = "understory/0"
            accept_header = "*/*"
            # accept_header = "text/html;q=0.9,*/*;q=0.8"
            _headers = {"user-agent": user_agent, "accept": accept_header}
            if headers:
    ...
    def card(self):
        return Semantics(
            mf.representative_card(self.mf2json.data, source_url=str(self.url))
        )
    ...
    def entry(self):
        return Semantics(
            mf.interpret_entry(self.mf2json.data, source_url=str(self.url))
        )
