...
class EsHandler(object):
    ...
    def __init__(self,
                 hdfs_url='http://10.162.80.7:50070',
        ...
        self.cms_label_index = 'sany_data_label'
    ...
    def get_files(self, data_type, farm, start_time, end_time, turbine=None):
        ...
        @return: 匹配到的所有文件列表
        """
        if data_type not in ['history', 'event', 'sec', 'fault', 'qd', 'cms', 'ems_log', 'qd-gslb']:
            return
        if not isinstance(turbine, list):
            print("请传入列表格式机组！")
            return
