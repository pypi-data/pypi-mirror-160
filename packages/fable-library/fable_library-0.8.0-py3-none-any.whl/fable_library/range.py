...
def make_range_step_function(step: _T, stop: _T, zero: _T, add: Any) -> Callable[[_T], Optional[Tuple[_T, _T]]]:
    ...
    def arrow_129(x: Optional[_T]=None, step: _T=step, stop: _T=stop, zero: _T=zero, add: Any=add) -> Optional[Tuple[_T, _T]]:
...
def integral_range_step(start: _T, step: _T, stop: _T, zero: _T, add: Any) -> IEnumerable[_T]:
    ...
    def arrow_130(start: _T=start, step: _T=step, stop: _T=stop, zero: _T=zero, add: Any=add) -> IEnumerable[_T]:
        return unfold(step_fn, start)

    return delay(arrow_130)


def range_big_int(start: int, step: int, stop: int) -> IEnumerable[int]:
    def arrow_131(x: int, y: int, start: int=start, step: int=step, stop: int=stop) -> int:
        return op_addition(x, y)

    return integral_range_step(start, step, stop, from_zero(), arrow_131)


def range_decimal(start: Decimal, step: Decimal, stop: Decimal) -> IEnumerable[Decimal]:
    def arrow_132(x: Decimal, y: Decimal, start: Decimal=start, step: Decimal=step, stop: Decimal=stop) -> Decimal:
        return op_addition_1(x, y)

    return integral_range_step(start, step, stop, from_parts(0, 0, 0, False, 0), arrow_132)


def range_double(start: float, step: float, stop: float) -> IEnumerable[float]:
    def arrow_133(x: float, y: float, start: float=start, step: float=step, stop: float=stop) -> float:
        return x + y

    return integral_range_step(start, step, stop, 0, arrow_133)


def range_int64(start: int, step: int, stop: int) -> IEnumerable[int]:
    def arrow_134(x: int, y: int, start: int=start, step: int=step, stop: int=stop) -> int:
        return op_addition_2(x, y)

    return integral_range_step(start, step, stop, 0, arrow_134)


def range_uint64(start: int, step: int, stop: int) -> IEnumerable[int]:
    def arrow_135(x: int, y: int, start: int=start, step: int=step, stop: int=stop) -> int:
        return op_addition_2(x, y)

    return integral_range_step(start, step, stop, 0, arrow_135)
...
def range_char(start: str, stop: str) -> IEnumerable[str]:
    ...
    def arrow_136(start: str=start, stop: str=stop) -> IEnumerable[str]:
        return unfold(step_fn, char_code_at(start, 0))

    return delay(arrow_136)
