...
___31124 = TypeVar("___31124")

___31123 = TypeVar("___31123")

___31192 = TypeVar("___31192")

___31190 = TypeVar("___31190")

___31070 = TypeVar("___31070")

___31068 = TypeVar("___31068")

___31097 = TypeVar("___31097")

___31096 = TypeVar("___31096")
...
class FSharpList(Record, Generic[_T]):
    ...
    def __eq__(self, other: Any=None) -> bool:
        ...
        if xs is other:
            ...
            def loop(xs_1_mut: FSharpList[___31124], ys_1_mut: FSharpList[___31124]) -> bool:
                while True:
                    (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                    matchValue : Optional[FSharpList[___31124]] = xs_1.tail
                    matchValue_1 : Optional[FSharpList[___31124]] = ys_1.tail
                    if matchValue is not None:
                        if matchValue_1 is not None:
                            xt : FSharpList[___31124] = matchValue
                            yt : FSharpList[___31124] = matchValue_1
            ...
            loop : Callable[[FSharpList[___31123], FSharpList[___31123]], bool] = loop
    ...
    def CompareTo(self, other: Any=None) -> int:
        xs : FSharpList[_T] = self
        def loop(xs_1_mut: FSharpList[___31192], ys_1_mut: FSharpList[___31192]) -> int:
            while True:
                (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                matchValue : Optional[FSharpList[___31192]] = xs_1.tail
                matchValue_1 : Optional[FSharpList[___31192]] = ys_1.tail
                if matchValue is not None:
                    if matchValue_1 is not None:
                        xt : FSharpList[___31192] = matchValue
                        yt : FSharpList[___31192] = matchValue_1
...
def FSharpList__get_Length(xs: FSharpList[_T]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[___31070], xs: FSharpList[_T]=xs) -> int:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Optional[FSharpList[___31070]] = xs_1.tail
            if match_value is not None:
                i_mut = i + 1
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[_T], index: int) -> _T:
    def loop(i_mut: int, xs_1_mut: FSharpList[___31097], xs: FSharpList[_T]=xs, index: int=index) -> ___31097:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value : Optional[FSharpList[___31097]] = xs_1.tail
            if match_value is not None:
                if i == index:
                    return xs_1.head
...
def for_all(f: Callable[[__A], bool], xs: FSharpList[__A]) -> bool:
    def arrow_93(acc: bool, x: __A, f: Callable[[__A], bool]=f, xs: FSharpList[__A]=xs) -> bool:
        return f(x) if acc else False

    return fold(arrow_93, True, xs)


def for_all2(f: Any, xs: FSharpList[__A], ys: FSharpList[__B]) -> bool:
    def arrow_94(acc: bool, x: __A, y: __B, f: Any=f, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> bool:
        return f(x, y) if acc else False

    return fold2(arrow_94, True, xs, ys)
...
def unzip(xs: FSharpList[Tuple[__A, __B]]) -> Tuple[FSharpList[__A], FSharpList[__B]]:
    def arrow_95(tupled_arg: Tuple[__A, __B], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B]], xs: FSharpList[Tuple[__A, __B]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B]]:
        return (FSharpList_Cons_305B8EAC(tupled_arg[0], tupled_arg_1[0]), FSharpList_Cons_305B8EAC(tupled_arg[1], tupled_arg_1[1]))

    return fold_back(arrow_95, xs, (FSharpList_get_Empty(), FSharpList_get_Empty()))


def unzip3(xs: FSharpList[Tuple[__A, __B, __C]]) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
    def arrow_96(tupled_arg: Tuple[__A, __B, __C], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]], xs: FSharpList[Tuple[__A, __B, __C]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
    ...
    return fold_back(arrow_96, xs, (FSharpList_get_Empty(), FSharpList_get_Empty(), FSharpList_get_Empty()))


def zip(xs: FSharpList[__A], ys: FSharpList[__B]) -> FSharpList[Tuple[__A, __B]]:
    def arrow_97(x: __A, y: __B, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> Tuple[__A, __B]:
        return (x, y)

    return map2(arrow_97, xs, ys)


def zip3(xs: FSharpList[__A], ys: FSharpList[__B], zs: FSharpList[__C]) -> FSharpList[Tuple[__A, __B, __C]]:
    def arrow_98(x: __A, y: __B, z: __C, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys, zs: FSharpList[__C]=zs) -> Tuple[__A, __B, __C]:
        return (x, y, z)

    return map3(arrow_98, xs, ys, zs)
...
def sort(xs: FSharpList[_T], comparer: IComparer[_T]) -> FSharpList[_T]:
    def arrow_99(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> int:
        return comparer.Compare(x, y)

    return sort_with(arrow_99, xs)


def sort_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> FSharpList[_T]:
    def arrow_100(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> int:
        return comparer.Compare(projection(x), projection(y))

    return sort_with(arrow_100, xs)


def sort_descending(xs: FSharpList[_T], comparer: IComparer[_T]) -> FSharpList[_T]:
    def arrow_101(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> int:
        return comparer.Compare(x, y) * -1

    return sort_with(arrow_101, xs)


def sort_by_descending(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> FSharpList[_T]:
    def arrow_102(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> int:
        return comparer.Compare(projection(x), projection(y)) * -1

    return sort_with(arrow_102, xs)


def sum(xs: FSharpList[_T], adder: IGenericAdder_1[_T]) -> _T:
    def arrow_103(acc: _T, x: _T, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_T]=adder) -> _T:
        return adder.Add(acc, x)

    return fold(arrow_103, adder.GetZero(), xs)


def sum_by(f: Callable[[_T], _U], xs: FSharpList[_T], adder: IGenericAdder_1[_U]) -> _U:
    def arrow_104(acc: _U, x: _T, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_U]=adder) -> _U:
        return adder.Add(acc, f(x))

    return fold(arrow_104, adder.GetZero(), xs)


def max_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> _T:
    def arrow_105(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> _T:
        return y if (comparer.Compare(projection(y), projection(x)) > 0) else x

    return reduce(arrow_105, xs)


def max(xs: FSharpList[_T], comparer: IComparer[_T]) -> _T:
    def arrow_106(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> _T:
        return y if (comparer.Compare(y, x) > 0) else x

    return reduce(arrow_106, xs)


def min_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer[_U]) -> _T:
    def arrow_107(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer[_U]=comparer) -> _T:
        return x if (comparer.Compare(projection(y), projection(x)) > 0) else y

    return reduce(arrow_107, xs)


def min(xs: FSharpList[_T], comparer: IComparer[_T]) -> _T:
    def arrow_108(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer[_T]=comparer) -> _T:
        return x if (comparer.Compare(y, x) > 0) else y

    return reduce(arrow_108, xs)
...
def average_by(f: Callable[[_T], _U], xs: FSharpList[_T], averager: IGenericAverager_1[_U]) -> _U:
    count : int = 0
    def arrow_109(acc: _U, x: _T, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, averager: IGenericAverager_1[_U]=averager) -> _U:
        nonlocal count
        count = (count + 1) or 0
        return averager.Add(acc, f(x))

    total : _U = fold(arrow_109, averager.GetZero(), xs)
    return averager.DivideByInt(total, count)
...
def all_pairs(xs: FSharpList[_T1], ys: FSharpList[_T2]) -> FSharpList[Tuple[_T1, _T2]]:
    ...
    def arrow_112(x: Optional[_T1]=None, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> None:
        def arrow_111(y: Optional[_T2]=None) -> None:
            nonlocal node
            def arrow_110(__unit: Any=None) -> FSharpList[Tuple[_T1, _T2]]:
        ...
        iterate(arrow_111, ys)

    iterate(arrow_112, xs)
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_113(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
    ...
    def loop(acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], predicate: Callable[[_T], bool]=predicate, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def arrow_114(acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def arrow_115(i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t : FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
...
def insert_at(index: int, y: _T, xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    def arrow_116(index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(result if is_done else (FSharpList_Cons_305B8EAC(y, result) if ((i + 1) == index) else arrow_116()))
...
def insert_many_at(index: int, ys: IEnumerable[_T], xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    result : FSharpList[_T] = fold(folder, FSharpList_get_Empty(), xs)
    def arrow_117(index: int=index, ys: IEnumerable[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(result if is_done else (append(ys_1, result) if ((i + 1) == index) else arrow_117()))
