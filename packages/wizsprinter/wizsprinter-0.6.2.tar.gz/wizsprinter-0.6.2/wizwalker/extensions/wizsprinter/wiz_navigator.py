...
async def toZone(clients, destinationZone):
    ...
    objectLines = await parseFile("traversalData/objectLocations.txt", worldName)
    interactiveTeleportersOriginal = await parseFile("traversalData/interactiveTeleporters.txt", worldName)
    uniqueObjectLocationsOriginal = await parseFile("traversalData/uniqueObjectLocations.txt", worldName)
...
async def main(clientHandler):
    ...
    try:
        await toZone(clients, 'WizardCity/WC_Streets/WC_OldeTown')
    finally:
