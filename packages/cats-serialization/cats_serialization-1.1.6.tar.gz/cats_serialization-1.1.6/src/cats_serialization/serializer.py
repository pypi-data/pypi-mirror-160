...
def _member_factory_wrapper(member):
    return lambda d, k: _tuple(member(d), None)


def _function_factory_wrapper(member, self):
    return lambda d, k: _tuple(member(self, d), None)
...
def _parse_annotation(annotation_data, default=Undefined):
    if type(annotation_data) is type:
        if _is_type_support(annotation_data):
            return ObjectField(origin=annotation_data, is_required=default is Undefined, default=default)
        else:
            return Field(is_type=annotation_data, default=default)
        ...
        if origin is UnionType:
            ...
            return IterableField(origin=field_prototype, is_required=default is Undefined, default=default)

    return Field()


def _parse_annotations(annotations_data, defaults):
    return {key: _parse_annotation(field, getattr(defaults, key, Undefined)) for key, field in annotations_data.items()}
...
class BaseSerializer(AbstractSerializer):
    ...
    def __cache__(cls):
        ...
        annotation_fields = _parse_annotations(annotations_data, cls)
        ...
        for key, member in members:
            ...
            if isinstance(member, BaseField):
                ...
                cached_fields[key] = MethodField(factory=_function_factory_wrapper(member, cls))
            else:
                # non callable fields
                cached_fields[key] = Field(default=member)
