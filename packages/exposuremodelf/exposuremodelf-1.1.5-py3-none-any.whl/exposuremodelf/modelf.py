...
print("model.load(num_epochs, data) where data = directory with training images")
print("model.eval(images, pretrained = True) where images = an array of image paths for model to rate")
...
class OpiDetectionDatasetR(Dataset):
    ...
    def __getitem__(self, index):
        #ImgDataset\Image#.jpg
        img_path = os.path.join(self.root_dir, self.annotations.iloc[index, 3])
        img_path = img_path[11:len(img_path)]
        with resources.path("exposuremodelf", img_path) as fp:
            img = Image.open(fp).convert("RGB")
        y_label = torch.tensor(float((self.annotations.iloc[index, 2]) - 1) / 4)
        if self.transform is not None:
            img = self.transform(img)

        return (img, y_label)

def load(num_epochs, data):
    ...
    dataset = OpiDetectionDatasetR(csv_file = 'ratings.csv', root_dir = data, transform = test_transforms)
...
def eval(images, pretrain = True):
    ...
    try:
        ...
        print("No model found. Load new model with model.load(num_epochs, data)")
