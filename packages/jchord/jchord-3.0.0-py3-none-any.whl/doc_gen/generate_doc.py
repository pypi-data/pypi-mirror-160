...
def assemble_table_of_contents(data):
    ...
    for module in data["modules"]:
        output.append(
            f"* <a href='#{module['name'].replace('.', '')}'>`{module['name']}`</a>"
        )
        for class_ in module["classes"]:
            ...
            for method in class_["methods"]:
                output.append(
                    f"        * Method `{class_['name']}.{method['signature']}`"
                )
        for function in module["functions"]:
            output.append(f"    * Function `{function['signature']}`")
        for exception in module["exceptions"]:
            output.append(f"    * Exception `{exception['name']}`")
    return "\n".join(output)
