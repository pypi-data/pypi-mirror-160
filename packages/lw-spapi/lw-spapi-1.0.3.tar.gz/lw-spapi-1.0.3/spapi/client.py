...
from spapi.exceptions import SPAPIClientIdNotInWhiteListException, get_exception_for_code, AuthorizationError
from spapi.lock import rl
...
class Client(object):
    ...
    def __init__(self, credentials, max_retries=3, is_grantless=False):
        """
    ...
    def get_params(self, url, **kwargs):
        return {
            'auth': self.auth,
            'headers': self._merge_headers(url, kwargs),
            'data': self._merge_data(kwargs)
        }
...
class AmazonSpApi(Client):
    ...
    def __init__(self, credentials, is_grantless=False):
        super(AmazonSpApi, self).__init__(credentials, is_grantless=is_grantless)
    ...
    def _lock_get_data(self, key_prefix, func, data_ttl, lock_ttl=1000 * 60 * 10):
        ...
        while True:
            ...
            if not data:
                locked = rl.lock(lock_key, lock_ttl)
                if locked:
                    try:
                        data = func()
