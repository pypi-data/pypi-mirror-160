...
from sqlalchemy import FLOAT,INT,VARCHAR
import varname
...
class pure_sunbath():
    ...
    def get_single_day_factor(self,func:function,day:int)->pd.DataFrame:
    ...
    def get_daily_factor_alter(self,func:function,startdate:int,enddate:int)->pd.DataFrame:
        ...
        try:
            ...
            if self.daily_factors.index.max()<now_minute_data:
                ...
                start_date_update=int(
                    datetime.datetime.strftime(self.daily_factors.index.max()+pd.Timedelta('1 day'),'%Y%m%d'))
                end_date_update=int(datetime.datetime.strftime(now_minute_data,'%Y%m%d'))
                calen=download_calendar(startdate=start_date_update,enddate=end_date_update)
...
class sqlConfig(object):
    def __init__(self,db_name:str=None,db_user:str='root',db_host:str='127.0.0.1',db_port:int=3306,db_password:str='Kingwila98'):
    ...
    def show_tables(self,db_name:str=None,full:bool=True):
        ...
        if db_name is None:
            db_name=self.db_name
        if full:
            res=pd.read_sql(f"select * from information_schema.tables where TABLE_SCHEMA='{db_name}'",con=engine)
        else:
            res=pd.read_sql(f"select TABLE_NAME from information_schema.tables where TABLE_SCHEMA='{db_name}'",con=engine)
    ...
    def show_databases(self,user_only:bool=True,show_number:bool=True)->list:
        '''显示数据库信息'''
        mycursor=self.login()
        res=pd.read_sql('select SCHEMA_NAME from information_schema.schemata',con=self.engine)
...
def database_update_minute_files(startdate:str=None,enddate:str=None,to_mat=True):
    '''
    ...
    for stock in tqdm.tqdm(stocks,desc='正在下载分钟数据'):
        ...
        df_sql=df_sql.drop(columns=['date'])
    ...
    for day in calen[:-1]:
        ...
        m.to_sql(name=str(day),con=sa.engine,if_exists='replace',index=False,
                  dtype={'open':FLOAT(2),'high':FLOAT(2),'low':FLOAT(2),'close':FLOAT(2),'amount':INT,'money':FLOAT(2),'num':INT,'code':VARCHAR})
