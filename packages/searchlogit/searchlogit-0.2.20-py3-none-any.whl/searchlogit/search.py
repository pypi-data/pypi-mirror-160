...
class Search():
    ...
    def check_sol_already_generated(self, har_mem, sol):
        # pop sol_num from har_mem
        new_har_mem = []
        for sol_i in har_mem:
            tmp_sol = sol_i.copy()
            ...
            new_har_mem.append(sol_i)

        sol.pop('sol_num', None)

        sol_already_generated = sol in new_har_mem

        # TODO? logic for lcm when generated vars not in class_params_spec...

        return sol_already_generated
    ...
    def initialize_memory(self, HMS):
        ...
        while dummy_iter < 30000:
            ...
            all_HM = HM + opp_HM
            sol_already_generated = self.check_sol_already_generated(all_HM, sol)
            if not sol_already_generated:
            ...
            # TODO! Reincluded new (random generated?) opp_sol
            opp_sol = self.generate_sol()

            all_HM = HM + opp_HM
            sol_already_generated = self.check_sol_already_generated(all_HM, opp_sol)
            if not sol_already_generated:
    ...
    def assess_sol(self, solution, har_mem):
        ...
        # all_HM = HM + opp_HM
        sol_already_generated = self.check_sol_already_generated(har_mem, solution)

        if not sol_already_generated:
            improved_sol, conv = self.evaluate_objective_function(solution)
        else:
            improved_sol, conv = solution, False
        if conv:
