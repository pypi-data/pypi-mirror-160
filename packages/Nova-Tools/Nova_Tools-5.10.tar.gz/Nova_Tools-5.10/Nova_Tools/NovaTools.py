...
class Tools:
    ...
    def login_amazon(email: str, password: str) -> str:
        ...
        if "You indicated you are a new customer, but an account already exists with the e-mail" in res:
            ...
            return {'email_username': str(email), 'password': str(password), 'The resulting': 'True'}
    def get_tiktok_sessionid(email: str, password: str) -> str:
        ...
        if ("user_id") in res.text:
            ...
            return {'username': str(email), 'password': str(password), 'The resulting': 'True', 'SessionId': sessionid}

        elif ("Incorrect account or password") in res.text:
            {'username': str(email), 'password': str(password), 'The resulting' :'Error Password Or Username'}
        else:
            {'username': str(email), 'password': str(password), 'The resulting' :'False'}
