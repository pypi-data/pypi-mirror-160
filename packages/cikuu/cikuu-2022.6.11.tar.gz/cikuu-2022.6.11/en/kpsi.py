...
def walk(sid, doc): 
	...
	[add( f"{t.lemma_}:POS:{t.pos_}", f"{t.lemma_}:LEX:{t.text.lower()}", f"LEM:{t.lemma_.lower()}", f"LEX:{t.text.lower()}", f"{t.pos_}:{t.lemma_.lower()}"
		,f"{t.lemma_.lower()}:{t.pos_}:{t.tag_}:{t.text.lower()}",f"{t.lemma_.lower()}:{t.pos_}:{t.tag_}",f"*:{t.pos_}:{t.tag_}", f"{t.lemma_.lower()}:{t.pos_}") for t in doc if not t.pos_ in ('PROPN','X', 'PUNCT') and t.is_alpha ]
	for t in doc:
		add( "#LEX", f"#{t.pos_}", f"#{t.tag_}",f"#{t.dep_}",) # book:VERB:VBG
		if t.pos_ not in ("PROPN","PUNCT","SPACE") and t.is_alpha and t.head.is_alpha: #*:VERB:~punct:VERB:wink
			reg(f"{t.lemma_.lower()}:{t.pos_}:{t.tag_}:{t.text.lower()}", sid) 
			add(f"{t.head.lemma_}:{t.head.pos_}:{t.dep_}:{t.pos_}:{t.lemma_}", f"{t.head.lemma_}:{t.head.pos_}:{t.dep_}", f"*:{t.head.pos_}:{t.dep_}")
			reg(f"{t.head.lemma_}:{t.head.pos_}:{t.dep_}:{t.pos_}:{t.lemma_}", sid)
			if t.dep_ not in ('ROOT'): 
				add(f"{t.lemma_}:{t.pos_}:~{t.dep_}:{t.head.pos_}:{t.head.lemma_}", f"{t.lemma_}:{t.pos_}:~{t.dep_}", f"*:{t.pos_}:~{t.dep_}")
	...
	for name,ibeg,iend in en.post_np_matcher(doc): #added 2022.7.25
		if name == 'v_n_vbn': 
			add(f"{doc[ibeg].lemma_}:{doc[ibeg].pos_}:v_n_vbn:{doc[ibeg].lemma_}:{doc[ibeg+1].text}:{doc[ibeg+2].text}", f"{doc[ibeg].lemma_}:{doc[ibeg].pos_}:v_n_vbn", f"*:{doc[ibeg].pos_}:v_n_vbn")
			reg(f"{doc[ibeg].lemma_}:{doc[ibeg].pos_}:v_n_vbn:{doc[ibeg].lemma_}:{doc[ibeg+1].text}:{doc[ibeg+2].text}", sid)
...
def mysql(dbfile, host='127.0.0.1', port=3307, cutoff=0):  
	...
	with my_conn.cursor() as cursor: 
		...
		cursor.execute(f"CREATE TABLE if not exists {name}(s varchar(128) COLLATE latin1_bin not null primary key, i int not null default 0, t text not null default '') engine=myisam  DEFAULT CHARSET=latin1 COLLATE=latin1_bin")
