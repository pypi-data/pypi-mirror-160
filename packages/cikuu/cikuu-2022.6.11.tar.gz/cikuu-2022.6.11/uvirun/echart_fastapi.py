...
def echart_bar(xdata:str="衬衫,羊毛衫,雪纺衫,裤子,高跟鞋,袜子", legends:str="商家A,商家B", ydata:str="[[5, 20, 36, 10, 75, 90],[5, 20, 36, 10, 75, 95]]", width:int=600, height:int=600,title="", subtitle=""):
	''' 柱状图， assure 数组长度相同  '''
	...
	values = json.loads(ydata) 
	for i, legend in enumerate(legends.strip().split(',')):
		bar.add_yaxis(legend, values[i])#bar.add_yaxis("商家B", [5, 20, 36, 10, 75, 95])
