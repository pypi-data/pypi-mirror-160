...
def ding_talk_webhook_message_is_str_valid(content):
    """
    非空字符串
    :param content: 字符串
...
class DingTalkWebhookMessage(object):
    """
    ...
    def __init__(self, webhook_url: str):
        ...
        机器人初始化
        """
        super(DingTalkWebhookMessage, self).__init__()
    ...
    def send_text(self, msg, is_at_all=False, at_mobiles=None, at_dingtalk_ids=None):
        ...
        if ding_talk_webhook_message_is_str_valid(msg):
    ...
    def send_image(self, pic_url):
        ...
        :return: 返回消息发送结果
        """
        if ding_talk_webhook_message_is_str_valid(pic_url):
            data = {
                "msgtype": "image",
                "image": {
                    "picURL": pic_url
    ...
    def send_link(self, title, text, message_url, pic_url=''):
        ...
        """
        if ding_talk_webhook_message_is_str_valid(title) and ding_talk_webhook_message_is_str_valid(text) and ding_talk_webhook_message_is_str_valid(message_url):
            data = {
                    "msgtype": "link",
                    "link": {
                        "text": text,
    ...
    def send_markdown(self, title, text, is_at_all=False, at_mobiles=None, at_dingtalk_ids=None):
        ...
        :return: 返回消息发送结果
        """
        if ding_talk_webhook_message_is_str_valid(title) and ding_talk_webhook_message_is_str_valid(text):
            data_at = dict()
            data = {
                "msgtype": "markdown",
                "markdown": {
    ...
    def send_action_card(self, action_card):
        ...
        :return: 返回消息发送结果
        """
        if isinstance(action_card, DingTalkWebhookMessageActionCard):
    ...
    def send_feed_card(self, links):
        ...
        link_data_list = []
        for link in links:
            if isinstance(link, DingTalkWebhookMessageFeedLink) or isinstance(link, DingTalkWebhookMessageCardItem):
...
class DingTalkWebhookMessageActionCard(object):
    """
    ...
    def __init__(self, title, text, btn_list, btn_orientation=0, hide_avatar=0):
        ...
        """
        super(DingTalkWebhookMessageActionCard, self).__init__()
        self.title = title
        self.text = text
        ...
        btn_valid_list = []
        for btn in btn_list:
            if isinstance(btn, DingTalkWebhookMessageCardItem):
    ...
    def get_data(self):
        ...
        """
        if ding_talk_webhook_message_is_str_valid(self.title) and ding_talk_webhook_message_is_str_valid(self.text) and len(self.btns):
            if len(self.btns) == 1:
                # 整体跳转ActionCard类型
                data = {
                        "msgtype": "actionCard",
...
class DingTalkWebhookMessageFeedLink(object):
    """
    FeedCard类型单条消息格式
    """
    def __init__(self, title, message_url, pic_url):
        ...
        """
        super(DingTalkWebhookMessageFeedLink, self).__init__()
        self.title = title
    ...
    def get_data(self):
        ...
        """
        if ding_talk_webhook_message_is_str_valid(self.title) and ding_talk_webhook_message_is_str_valid(self.message_url) and ding_talk_webhook_message_is_str_valid(self.pic_url):
            data = {
                    "title": self.title,
...
class DingTalkWebhookMessageCardItem(object):
    """
    ...
    def __init__(self, title, url, pic_url=None):
        ...
        """
        super(DingTalkWebhookMessageCardItem, self).__init__()
        self.title = title
        self.url = url
        self.pic_url = pic_url
    ...
    def get_data(self):
        ...
        @return: 子控件的数据
        """
        if ding_talk_webhook_message_is_str_valid(self.pic_url) and ding_talk_webhook_message_is_str_valid(self.title) and ding_talk_webhook_message_is_str_valid(self.url):
            # FeedCard类型
            data = {
                "title": self.title,
                "messageURL": self.url,
            ...
            return data
        elif ding_talk_webhook_message_is_str_valid(self.title) and ding_talk_webhook_message_is_str_valid(self.url):
            # ActionCard类型
            data = {
                "title": self.title,
                "actionURL": self.url
...
class DingTalkWebhookMessageQueue(object):
