...
def get_networks_router(network_store: NetworkStore):
    ...
    async def delete_links(
        ...
        uids: Optional[List[str]] = Body(
            None, description="Limit deletion to a set of `uid`s", embed=True
        ),
        group_uids: Optional[List[str]] = Body(
            None, description="Limit deletion to a set of `group_uid`s", embed=True
        ),
        purge: bool = Query(
            False, description="Permanently delete elements (history is lost)"
        ),
    ...
    async def delete_nodes(
        ...
        remove_associated_links: bool = Query(
            False,
            description="While this is the endpoint for deleting nodes,"
            " you can also use this endpoint delete links. If you set"
            " `remove_associated_links` to `True`, the endpoint will also attempt"
            " to delete links whose `source_uid` or `target_uid` matches the entries"
            " in the `uids` field in addition to the nodes it will delete. Note that"
            " this feature is deprecated and is not (nor has never been) tested.",
            deprecated=True,
        ),
        purge: bool = Query(
            False, description="Permanently delete elements (history is lost)"
        ...
        if remove_associated_links:
            links = await network_store.get_links(
                network_uid, adjacent_nodes=uids, only_interior=False
            )
            if links:
                await network_store.drop_nodes(
                    network_uid,
                    uids=[link["uid"] for link in links],
                    purge=purge,
                    dt=datetime,
                )
