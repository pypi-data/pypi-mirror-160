...
class SquareMesh(SquareMeshBase):
    ...
    def extract_hData(self, y, result):
        """
        Extract a horizontal line data. Data number equals to segment_num[0].
        Direction is left to right.
        :param y: Y coordinate. | float.
        :param result: Result field to be extracted.
        :return: Data arrays. | np.ndarray.
        """
        data = np.zeros(self._segment_num[0], dtype=np.float64)

        exist = False
        index = 0
        for i in range(self._cell_num):
            if abs(self._cell_coord[i, 1] - y)<1e-10:
                exist = True
                data[index] = result[i]
                index += 1
    ...
    def extract_vData(self, x, result):
        """
        Extract a vertical line data. Data number equals to segment_num[1].
        Direction is lower to upper.
        :param x: X coordinate. | float.
        :param result: Result field to be extracted.
        :return: Data arrays. | np.ndarry.
        """
        data = np.zeros(self._segment_num[1], dtype=np.float64)

        exist = False
        index = 0
        for i in range(self._cell_num):
            if abs(self._cell_coord[i, 0] - x)<1e-10:
                exist = True
                data[index] = result[i]
                index += 1
