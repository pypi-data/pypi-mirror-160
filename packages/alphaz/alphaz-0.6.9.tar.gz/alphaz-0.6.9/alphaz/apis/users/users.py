...
from ...utils.api import ADMIN_USER_ID_PUBLIC
...
def try_register_user(
    ...
    if (
        ...
        if (
            LOGIN_MODE != LoginModes.NO_CONFIRMATION.value
            and (
                "id" in userByMail
...
def try_login(username: str, password: str, su: bool = False):
    ...
    if not DB.add_or_update(
        UserSession(
            user_id=user_data["id"], token=token, ip=request.remote_addr, expire=expire,
        )
    ):
...
def try_su_login(admin_user_id: str = None, admin_user_name: str = None):
    if admin_user_id is None and admin_user_name is not None:
        admin_user = user_lib.get_user_data_by_username(admin_user_name)
        if admin_user is None:
            raise AlphaException(f"Admin user {admin_user_name} does not exist")
        admin_user_id = admin_user["id"]

        API.set(ADMIN_USER_ID_PUBLIC.name, admin_user_id)
    output = try_login(None, None, su=True)
