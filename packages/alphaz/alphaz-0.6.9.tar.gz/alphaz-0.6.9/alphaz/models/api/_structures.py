...
class AlphaFlask(Flask):
    ...
    def get_logged_user(self, algorithms=["HS256"]):
        ...
        admin_users = core.api.conf.get("admins/users")
        ...
        if token is not None:
            ...
            and self[ADMIN_USER_ID_PUBLIC.name] is not None
        ):
            from ...libs import user_lib  # todo: modify

            if admin_users is not None and self[ADMIN_USER_ID_PUBLIC.name] is not None:
                user = [
                    x
                    for x in admin_users
                    if ("id" in x and x["id"] == self[ADMIN_USER_ID_PUBLIC.name])
                ]
                user_data = user[0]
            user_data = (
                user_lib.get_user_data_by_id(self[ADMIN_USER_ID_PUBLIC.name])
                if user_data is None
                else user_data
            )
        elif (
            self.is_admin(check_logged_user=False)
            and self[ADMIN_USER_NAME_PUBLIC.name] is not None
        ):
            from ...libs import user_lib  # todo: modify
            ...
            if (
                ...
                user = [
                    ...
                    if (
                        "username" in x
                        and x["username"] == self[ADMIN_USER_NAME_PUBLIC.name]
                    )
                ]
                user_data = user[0]
            user_data = user_lib.get_user_data_by_username(
