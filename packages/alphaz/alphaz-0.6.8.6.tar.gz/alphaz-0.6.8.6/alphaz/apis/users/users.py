...
LOGIN_MODE = None if LOGIN_MODE is None else LOGIN_MODE.lower()
...
def try_register_user(
    ...
    if (
        ...
        if (
            LOGIN_MODE != LoginModes.NO_CONFIRMATION.lower()
            and (
                "id" in userByMail
...
def try_login(username: str, password: str, su: bool = False):
    ...
    if not su and (
        ...
        if LOGIN_MODE == LoginModes.LDAP.lower():
...
def try_su_login(admin_user_id: str = None, admin_user_name: str = None):
    if admin_user_id is None and admin_user_name is not None:
        ...
        API.set("admin_user_id", admin_user_id)
