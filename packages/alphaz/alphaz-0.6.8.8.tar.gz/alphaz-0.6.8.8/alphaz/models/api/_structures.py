...
class AlphaFlask(Flask):
    ...
    def set_error(
        ...
        self.get_current_route().set_error(
            message=message, description=description, warning=warning
        )
    ...
    def set_warning(
        self, message: typing.Union[str, AlphaException], description: str = None
    ):
        self.set_error(message=message, description=description, warning=1)
    ...
    def get_logged_user(self, algorithms=["HS256"]):
        ...
        if token is not None:
            ...
            self.is_admin(check_logged_user=False) and self["admin_user_id"] is not None
        ):
            from ...libs import user_lib  # todo: modify

            user_data = user_lib.get_user_data_by_id(self["admin_user_id"])
        return user_data
