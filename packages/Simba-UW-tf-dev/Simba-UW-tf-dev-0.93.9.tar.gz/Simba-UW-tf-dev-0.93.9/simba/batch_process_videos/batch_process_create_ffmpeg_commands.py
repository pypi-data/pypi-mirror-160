...
class FFMPEGCommandCreator(object):
    ...
    def clip_videos(self):
        ...
        for video, video_data in self.videos_to_clip.items():
            ...
            command = 'ffmpeg -i {} -ss {} -t {} -async 1 -c:a copy {} -hide_banner -loglevel error'.format(in_path, start_time, end_time, out_path)
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
    ...
    def apply_frame_count(self):
        ...
        for video, video_data in self.videos_to_frm_cnt.items():
            ...
            in_path, out_path = video_data['path'], os.path.join(self.process_dir, os.path.basename(video_data['path']))
            command = 'ffmpeg -i ' + in_path + ' -vf "drawtext=fontfile=Arial.ttf: ' + "text='%{frame_num}': start_number=1: x=(w-tw)/2: y=h-(2*lh): fontcolor=black: fontsize=20: box=1: boxcolor=white: boxborderw=5" + '" ' + "-c:a copy " + out_path + ' -hide_banner -loglevel error'
            subprocess.call(command, shell=True, stdout=subprocess.PIPE)
        self.replace_files_in_temp()
    ...
    def apply_clahe(self):
        ...
        for video, video_data in self.videos_to_frm_cnt.items():
            ...
            writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*'XVID'), fps, (width, height), 0)
            while True:
                ret, image = cap.read()
                if ret == True:
