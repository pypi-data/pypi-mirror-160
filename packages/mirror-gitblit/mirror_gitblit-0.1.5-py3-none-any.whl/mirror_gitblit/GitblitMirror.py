...
class Gitblit:
    ...
    def __init__(self, repository_json:str, web_info:dict, source_base_url:str, backup_dir:str):
        """
        Doc.

        :param repository_json: JSON-File, where repositories of this server are listed.
        :param web_info: A dictionary with 3 keys:
            * `source_web_url` URl to Web-interface of gitblit-server, for example `https://{host}.{domain}.{tld}/gitblit`
            * `user` Gitblit username, which has access to Web-interfaces
            * `password` Password of the given Gitblit user
        ...
        self._user = web_info.get("user", None)
        self._password = web_info.get("password", None)
        self._source_web_url = web_info["source_web_url"]
    ...
    def list_repositories(self):
        """
        download the list of repositories and save it to a file named by attribute `self.repository_json`
        """
    ...
    def clone_repos(self, repo_filter:Callable=None, remove_existing_destination:bool=False):
        ...
        for i, repos in enumerate(list_of_repos):
            clone_result = self._clone_mirror_repository(repos, remove_existing_destination)
