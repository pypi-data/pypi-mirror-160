...
DBLog = None  # pylint: disable=invalid-name


def initialise_orm(table_name: str, schema_name: str = ""):
    """
    Initialise SQLalchemy ORM object to interact with the database.

    Args:
        table_name (str): The name of the table
        schema_name (str): The schema of the table. Defaults to empty string.

    Raises:
        Exception: If an ORM representation if not possible, mostly in the cases of invalid schema.

    """

    additional_args = {}
    if schema_name:
        additional_args = {"schema": schema_name}

    global DBLog  # pylint: disable=W0603,invalid-name,
    ...
    try:
        ...
        class ORMTable(Base):
            ...
            __table_args__ = additional_args
        ...
        logger.exception('Unable to create an OR object')
        msg = (
            "Unable to create an ORM object. Please check the DB schema"
        )
...
class DBStore(BaseRunLogStore):
    ...
    def __init__(self, config):
        ...
        if 'table_name' not in self.config:
            msg = (
                'Run log store of db should have a table name as part of the config provided by table_name.'
                ' The schema can also be provided by schema_name.table_name'
            )
            raise Exception(msg)
        ...
        initialise_orm(self.table_name, self.schema_name)

    @property
    def table_name(self) -> str:
        """
        Returns the table name from the config

        Returns:
            str: The table name as per config
        """
        schema_and_table = self.config['table_name'].split('.')

        if len(schema_and_table) == 1:
            return schema_and_table

        return schema_and_table[1]

    @property
    def schema_name(self) -> str:
        """
        Returns the schema name as per config or None

        Returns:
            str: returns the schema name as per config
        """
        schema_and_table = self.config['table_name'].split('.')
