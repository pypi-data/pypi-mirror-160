...
"""
    ...
    def vks_send_key(self, fingerprint: str, keyserver: str) -> VksUploadResponse:
        """Upload a key from the user's local keyring to a Verifying Keyserver"""
        return vks_upload_key(
            key=self.export(fingerprint).decode(), keyserver=keyserver
        )
    ...
    def vks_recv_key(self, identifier: str, keyserver: str) -> None:
        """Download a key from a Verifying Keyserver to the user's local keyring

        :param identifier: Fingerprint, Key ID, or email address
        :param keyserver: keyserver url
        """
