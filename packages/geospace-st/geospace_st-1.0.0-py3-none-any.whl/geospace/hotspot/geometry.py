...
from geospace.hotspot import differential_expression, hotspot_decomposition
...
def geometric_markers(adata, inds, min_fc=1, use_decomp=False):
    ...
    for cur_ind in inds:
        ...
        gene_fc = np.full(fill_value=100, shape=len(gene_set))
        for other_ind in inds:
            ...
            try:
                gene_fc = np.minimum(gene_fc, var)
            except ValueError:
