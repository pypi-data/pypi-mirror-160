...
# ===========================================
from dw_chaco_etl.utils.core import settings

def get_dict_connection(name):
    x = importlib.import_module(settings.SETTINGS_MODULE)
    return getattr(x, "DATABASES")[name]

def get_conexion(name):
    return get_dict_connection(name)["AIRFLOW_PARAMETERS"]["AIRFLOW_NAME"]

def get_str_conexion_mssql(name):
	dict_conexion = get_dict_connection(name)
