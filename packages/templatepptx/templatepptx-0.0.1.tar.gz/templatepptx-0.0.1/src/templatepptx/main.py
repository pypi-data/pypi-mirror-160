"""
 ...
 Usage       : main.py --config "path\\to\\config\\file.json" --context "path\\to\\context_file.json"
...
#Custom Modules
from src.pptx_template.error import error, out_warning
...
def parse_template_ppt(ppt, context, output_path) -> Presentation:
    ...
    # Find template pptx
    try:
        ppt = Presentation(os.path.join(SCRIPT_DIR, "processing", ppt))
    except Exception as e:
...
def main():
    ...
    parser.add_argument('--context', type=str, help="File path to the context file", required=True)
    args = parser.parse_args()

    # Open and load config file
    try:
        with open(os.path.join(CONFIG_DIR, args.config), 'r') as config_file:
            config_data = json.load(config_file)
            template_ppt = config_data["in_pptx"]
            out_ppt = config_data["out_pptx"]
    except Exception as e:
        error(e, kill=True, message="Could not find config file")

    # Open and load context file
    try:
        with open(args.context, 'r') as context_file:
            context = json.load(context_file)
    except Exception as e:
        error(e, kill=True, message="Could not load context file")

    output_path = os.path.join(SCRIPT_DIR, "processing", "output", out_ppt)
    ppt = parse_template_ppt(template_ppt, context, output_path)
