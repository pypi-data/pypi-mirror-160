...
# Library loading
MB_LIB = load_json_lib("artefacts/model_brand.json")
BM_LIB = load_json_lib("artefacts/brand_model.json")
MODEL_LIB = sorted(load_lib("artefacts/model_lib.npy"), key=len, reverse=True)
BRAND_LIB = sorted(load_lib("artefacts/brand_lib.npy"), key=len, reverse=True)
...
PATTERNS = load_json_lib("artefacts/patterns.json")
...
def fuzzy_match_brand_model(matches, tokens):
    ...
    #Replace if model exist
    # if matches['model'] is not None:
    #     matches['brand'] = MB_LIB[matches['model']][0]
