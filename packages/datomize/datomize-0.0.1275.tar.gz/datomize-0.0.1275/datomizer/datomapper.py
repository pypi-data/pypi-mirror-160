...
from datomizer.helpers.common_helper import get_flow, wait_for_step_type, save_df_to_csv, validate_data_size_limits
...
class DatoMapper(object):
    ...
    def create_datasource(self, path, df, name, delimiter):
        with tempfile.TemporaryDirectory() as temp_dir:
            if df is not None:
                path = save_df_to_csv(df, temp_dir, name)
            validate_data_size_limits(path, delimiter)
            self.create_datasource_from_path(Path(path))
    ...
    def discover(self, path=None, df=None, df_name=None, sample_percent: int = 1, title: str = "sdk_flow",
        ...
        self.create_datasource(path, df, df_name if df_name else "temp_sdk", delimiter)
