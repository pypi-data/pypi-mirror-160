            ...
            try:
                ...
                if self.user_model.objects.filter(**{self.social_id_field:id}).exists():
                    ...
                    if self.active_field_name==None or getattr(user_instance, self.active_field_name):
                        ...
                        return False, None, '', ErrorManager.get_error_by_code(INVALID_CREDENTIALS)
                elif self.user_model.objects.filter(**{self.login_id_field_name:email, f'{self.social_id_field}__isnull':True}).exists():
                    user_instance=self.user_model.objects.get(**{self.login_id_field_name:email, f'{self.social_id_field}__isnull':True})
                    if self.active_field_name==None or getattr(user_instance, self.active_field_name):
                    ...
                    user_instance=self.user_model(**{self.social_id_field_name:id, self.name_field_name:nombre, self.login_id_field_name:email})
