...
# Limit the size client can read to 12MB
...
class Executor:
    ...
    def handle_request_run_application(self, request):
        ...
        try:
            ...
            # create output file
            filename = os.path.join(run_dir, "pwrapper-out.txt")
            try:
            ...
            try:
                ...
                r = subprocess.Popen(cmd, shell = True, stdout = out_f)
                launched = True
    ...
    # read file
    def handle_request_read_run_artifact(self, request):
        log_prefix = "handle_request_read_run_artifact"
        # request example
        # {
        #     "action": "read-run-artifact",
        #     "id": 123,
        #     "artifact_name": "foo"
        # }
        try:
            validate_model('read_run_artifact_input', request)
        except ValidationError as e:
            logger.exception(f"{log_prefix}: validation error, schema: read_run_artifact_input, error: {str(e)}")
            return {
                "status": SERVER_STATUS.INVALID_REQUEST_SCHEMA.value,
                "message": str(e)
            }
        ...
        id = request["id"]
        run_dir = os.path.join(self.runs_dir, str(id))
        filename = os.path.join(run_dir, request['artifact_name'])
        try:
            filesize = os.path.getsize(filename)
            if filesize >= MAX_FILE_SIZE:
                logger.exception(f"{log_prefix}: file: {filename}, size: {filesize}, it is too big")
                return {
                    "status": SERVER_STATUS.FILE_TOO_BIG.value,
                }
            ...
            logger.exception(f"{log_prefix}: unabel to read file {filename}, error: {str(e)}")
            return {
                "status": SERVER_STATUS.FILE_READ_FAILURE.value,
            }
    ...
    def handle_request(self, request):
        ...
        elif action == "read-run-artifact":
            response = self.handle_request_read_run_artifact(request)
        else:
