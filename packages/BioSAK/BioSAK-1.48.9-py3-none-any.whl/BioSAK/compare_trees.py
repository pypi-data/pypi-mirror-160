...
BioSAK compare_trees -t1 tree_dir -t2 tree_dir -tx tree
...
def compare_trees_worker(arg_list):
    ...
    if len(shared_leaves) == 0:
        ...
        print('Different leaves were found in %s (%s) and %s (%s), will perform mantel test based on %s shared leaves' % (
                tree1_basename, len(tree1_leaf_list), tree2_basename, len(tree2_leaf_list), len(shared_leaves)))
