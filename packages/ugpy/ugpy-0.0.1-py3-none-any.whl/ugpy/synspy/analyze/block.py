...
class BlockedAnalyzer(object):
    ...
    def __init__(self, image, synapse_diam_micron, vicinity_diam_micron, maskblur_micron, view_reduction,
        ...
        if desired_block_size is None:
            ...
            print("Using %s voxel preferred sub-block size. Override with ZYX_BLOCK_SIZE='int,int,int'" % (
                desired_block_size,))
        ...
        print(
            "Using %s viewing mode. Override with VIEW_MODE=raw or VIEW_MODE=dog." % (self.view_raw and 'raw' or 'dog'))

        self.image = image
        self.view_reduction = view_reduction

        self.kernels_3x1d, self.kernels_3d = prepare_kernels(image.micron_spacing, synapse_diam_micron,
                                                             vicinity_diam_micron, maskblur_micron)
        ...
        self.max_border_widths += np.where(
            ...
            np.array(self.view_reduction, dtype=np.int32) - self.max_border_widths % np.array(self.view_reduction,
                                                                                              dtype=np.int32),
        ...
        for d in range(3):
            ...
            if self.block_size[d] <= self.max_border_widths[d] * trim_factor:
                raise ValueError(
                    "Block size %s too small for filter borders %s" % (self.block_size, self.max_border_widths))
    ...
    def find_blocking(self, desired_block_size):
        ...
        def find_blocking_1d(d):
            if self.image.shape[d] < desired_block_size[d]:
                if self.image.shape[d] % self.view_reduction[d] == 0:
                    ...
                    raise ValueError(
                        "Dimension %d, length %d, smaller than desired block size %d but not divisible by reduction %d" % (
        ...
        for d in range(3):
            try:
                ...
                axis_size = self.view_reduction[d] * (
                        min(desired_block_size[d], self.image.shape[d]) // self.view_reduction[d])
    ...
    def volume_process(self):
        ...
        print(
            "Allocated %s %s view_image with %s voxel size for %s reduction of %s source image with %s voxel size." % (
                view_image.shape, view_image.dtype,
                list(map(lambda a, b: a * b, self.image.micron_spacing, self.view_reduction)), self.view_reduction,
                self.image.shape, self.image.micron_spacing))
