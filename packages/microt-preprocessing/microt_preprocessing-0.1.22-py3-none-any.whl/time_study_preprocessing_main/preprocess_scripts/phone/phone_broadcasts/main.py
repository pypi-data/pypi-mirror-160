        ...
        if path.exists(date_folder_path):
            ...
            if not path.exists(day_file_save_path) or not path.exists(day_screen_minute_save_path) or not path.exists(
                ...
                if hours_with_target_file != 0:
                    ...
                    df_day_minute = get_screen_status_matrix_minute(df_participant, date)
                    df_day_hour = get_screen_status_matrix_hour(df_day_minute)
                    df_day_minute.to_csv(day_screen_minute_save_path, index=False)
                    df_day_hour.to_csv(day_screen_hour_save_path, index=False)
