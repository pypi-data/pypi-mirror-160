...
class Model(object):
	...
	'''
		...
		we basically construct pending_updates query to mongo
	'''
		...
		for attr_name, attr_obj in self.__class__._attrs_.items():
			...
			if(attr_name not in doc):
				...
				if(_default != _OBJ_END_):
					...
					if(attr_obj._type == dict):
						_default = self.get_custom_dict(attr_name, {})  # copy
					elif(attr_obj._type == list):
						_default = self.get_custom_list(attr_name, [])  # copy
