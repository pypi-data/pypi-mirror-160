...
from .linenumbers import LineNumberBar
...
class CodeView(tkinter.Text):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().grid(row=0, column=2, sticky="nswe")

        self._line_numbers = LineNumberBar(self._frame, self)
        self._line_numbers.grid(row=0, column=0, rowspan=2, sticky="nswe")

        ttk.Separator(self._frame, orient="vertical").grid(row=0, column=1, sticky="ns")
        ...
        self._hs.grid(row=1, column=2, sticky="we")
        self._vs.grid(row=0, column=3, sticky="ns")
    ...
    def _cmd_proxy(self, command: str, *args) -> Any:
        ...
        if command == "insert":
            ...
            self._highlight()
        elif command in {"xview", "yview", "see"}:
            self.event_generate("<<Scroll>>")
    ...
    def _set_color_scheme(
        ...
        self.event_generate("<<ColorSchemeChanged>>")
        self.highlight_all()

    def _set_lexer(self, lexer: pygments.lexers.Lexer) -> None:
        self._lexer = lexer

        self.event_generate("<<LexerChanged>>")
        self.highlight_all()
