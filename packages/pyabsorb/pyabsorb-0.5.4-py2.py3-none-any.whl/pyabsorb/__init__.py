...
class Mixture:
    ...
    def mix_mue(self, energy, thickness_cm=0.01):
        ...
        self.transm = np.exp(-self.mu*self.thick)
        logger.info(f"""{'energy':<8s} {'% transm':<8s} {'mu:<8s'} {'muR':<8s} {"f'":<8s} {"f''":<8s} {'total xs':<15s} {'rayleigh xs':<15s} {'compton xs':<15s} {'photoelectric xs':<15s}""")
        for i in range(len(energy)):
            logger.info('%8d|%10.3f|%8.3f|%8.3f|%8.3f|%8.3f' \
