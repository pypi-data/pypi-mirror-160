...
class MCLIIntegration(SerializableDataclass, ABC):
    ...
    def build_docker(self) -> bool:
        """If True, will add the integration in docker and will not add directly to the job
        """
        try:
    ...
    @build_docker.setter
    def build_docker(self, val: bool) -> None:
        self._build_docker = val
    ...
    @classmethod
    def from_dict(cls: Type[T_SerializableDataclass],
                  data: Dict[str, Any],
                  build_docker: bool = False) -> T_SerializableDataclass:
        ...
        if integration_type == IntegrationType.wandb:
            ...
            # docker build only supported for local for now
            integration.build_docker = build_docker
        else:
