...
DatosList,Orden = Inicializar(3)##(1) roman, (2) bold, (3) typewriter, (4) para italic. #Comando para resetear las variables guardadas.

c_a = [sympy.symbols("a"),1.3]
S(c_a)

c_b = [sympy.symbols("b"),2.6]
S(c_b)

c_c = [sympy.symbols("c"),3.2]
S(c_c)

def Busqueda(symbolo):
  print("FUNCIONA BUSQUEDA----------------<------------------<-")
  print(symbolo)
  for c_element in DatosList:
    if c_element[0] == symbolo:
      print(c_element)
      if isinstance(c_element[1],float):#Si tenemos un numero
          return sympy.symbols(str(c_element[1]))
      elif isinstance(c_element[1],int):#Si tenemos un float
          return sympy.symbols(str(c_element[1]))
      elif c_element[1] != None:#Si tenemos una expresión
          return c_element[1]
      else:
        return c_element[1]#Si es None
  return symbolo


#print_latex(c_a[0]*c_b[0]/c_c[0])
...
class LatexPrinter(Printer):
    ...
    def _print_Symbol(self, expr, style='plain'):
        ...
        if expr in self._settings['symbol_names']:
            print("ZZZZZZZZ")
            print(self._settings['symbol_names'][expr])
