...
class Parameters:
    def __init__(self, name, ipeMaxVal, headingMaxVal, distanceToSwimMaxVal, distanceToPlatMaxVal,
                 distanceToSwimMaxVal2, distanceToPlatMaxVal2, corridorAverageMinVal, directedSearchMaxDistance,
                 focalMinDistance, focalMaxDistance, semiFocalMinDistance, semiFocalMaxDistance, corridoripeMaxVal,
                 annulusCounterMaxVal, quadrantTotalMaxVal, chainingMaxCoverage, percentTraversedMaxVal,
                 percentTraversedMinVal, distanceToCentreMaxVal, thigmoMinDistance, fullThigmoMinVal, smallThigmoMinVal,
                 ipeIndirectMaxVal, percentTraversedRandomMaxVal, headingIndirectMaxVal, useDirect, useFocal,
                 useDirected, useIndirect, useSemiFocal, useChaining, useScanning, useRandom, useThigmogaxis):
        self.name = name
        ...
        with open(filename, newline="") as file:
            try:
                ...
                messagebox.showinfo(None,
                                    "Please select the first X value (e.g. 23.45), then the first Y value, and finally the starting Time value.")
                top.attributes('-topmost', True)
                top.mainloop()
            except:
                top.attributes('-topmost', False)
...
def saveFileAsExperiment(software, filename, filedirectory):
    ...
    for filename in filenameList:
        ...
        if software == "ethovision":
            ...
            for fps, frame, x, y in zip(columns[fpsCol][1:], columns[frameCol][1:], columns[xCol][1:],
                                        columns[yCol][1:]):
                try:
