...
class AccessTokenAcquisitionGoalOptions(GoalSubsystem):
    ...
    for_ci = BoolOption(
        "--for-ci",
        default=False,
        help=f"Genrate a token with impersonation permissions (limited to org owners/admins) that can be used in CI. Implies `--{ACQUIRE_TOKEN_GOAL_NAME}-output={OutputType.CONSOLE.value}`.",
    )
...
class AuthStoreOptions(Subsystem):
    ...
    from_env_var = StrOption("--from-env-var", default=None, help="Loads the access token from an environment variable")
