...
from accelbrainbase.iteratabledata.ssda_transformer_iterator import SSDATransformerIterator as _SSDATransformerIterator
from accelbrainbase.noiseable_data import NoiseableData


class LabeledCSVSSDATransformerIterator(_SSDATransformerIterator):
    '''
    Iterator that draws from CSV files and generates `torch.Tensor` of labeled samples.
    '''
    ...
    def __init__(
        ...
        target_observed_arr, _ = labeled_csv_extractor.extract(target_csv_path)
        ...
        self.__target_observed_arr = target_observed_arr
    ...
    def generate_learned_samples(self):
        ...
        for _ in range(self.iter_n):
            ...
            target_row_arr = np.arange(self.__target_observed_arr.shape[0])
            np.random.shuffle(target_row_arr)

            target_batch_arr = self.__target_observed_arr[target_row_arr[:self.batch_size]]
            target_batch_arr = torch.from_numpy(target_batch_arr)
            target_batch_arr = target_batch_arr.to(self.__ctx).float()
            target_batch_arr = self.pre_normalize(target_batch_arr)

            if self.__noiseable_data is not None:
                training_batch_arr = self.__noiseable_data.noise(training_batch_arr)
                target_batch_arr = self.__noiseable_data.noise(target_batch_arr)

            train_encoded_observed_arr = training_batch_arr
            train_decoded_observed_arr = training_batch_arr
            train_encoded_masked_arr = None
            train_decoded_masked_arr = None
            test_encoded_observed_arr = test_batch_arr
            test_decoded_observed_arr = test_batch_arr
            test_encoded_masked_arr = None
            test_decoded_masked_arr = None

            list_tuple = self.create_pretext_task_samples_list(target_batch_arr)
            (
            ...
            yield (
                train_encoded_observed_arr,
                train_decoded_observed_arr,
                train_encoded_masked_arr,
                train_decoded_masked_arr,
                test_encoded_observed_arr,
                test_decoded_observed_arr,
                test_encoded_masked_arr,
                test_decoded_masked_arr,
                training_label_arr,
                test_label_arr,
                pretext_encoded_observed_arr_list, 
                pretext_decoded_observed_arr_list, 
