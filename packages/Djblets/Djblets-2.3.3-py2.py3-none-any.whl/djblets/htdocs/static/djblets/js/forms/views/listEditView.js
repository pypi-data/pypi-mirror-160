...
(function () {
  ...
  Djblets.Forms.ListEditView = Backbone.View.extend({
    ...
    render() {
      ...
      this._$list.find('.djblets-c-list-edit-widget__input').each((idx, el) => this._values.push($(el).val()));
    ...
    _addItem(e) {
      ...
      $entry.find('.djblets-c-list-edit-widget__add-item').on('click', e => this._addItem(e)).end().find('.djblets-c-list-edit-widget__input').on('change', e => this._onBlur(e)).end();
    ...
    _removeItem(e) {
      ...
      if (this._values.length > 1) {
        ...
        this._$list.find('.djblets-c-list-edit-widget__entry').each((idx, el) => $(el).attr('data-list-index', idx));
      } else {
        this._values[index] = '';
        $target.siblings('.djblets-c-list-edit-widget__input').val('');
      }

      this._$hidden.val(this._values.filter(v => v.length > 0).join(this._sep));
    },
    ...
    _onBlur(e) {
      const $target = $(e.target);
      const index = $target.closest('.djblets-c-list-edit-widget__entry').attr('data-list-index');
