...
def quick_modbus_single_register_decoder(ip,reg,nbs,dtype,unit=1,port=502):
    client2 = ModbusTcpClient(ip, port=port)### batH
    client.connect()
    result = client.read_holding_registers(reg, nbs, unit=unit)
    decoders={}
    decoders['bb'] = BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Big, wordorder=Endian.Big)
    decoders['bl'] = BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Big, wordorder=Endian.Little)
    decoders['lb'] = BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Little, wordorder=Endian.Big)
    decoders['ll'] = BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Little, wordorder=Endian.Little)
    values = [decode_register(d,dtype) for k,d in decoders.items()]
    # print('='*60+'\nvalues are \n',values)
    return values
...
def _vmuc_mbmap():
    vmuc_modebus_map = _parseXML_VMUC(FILE_VMUC_XML)
    vmuc_modebus_map = _makemodebus_mapUnique(vmuc_modebus_map)
