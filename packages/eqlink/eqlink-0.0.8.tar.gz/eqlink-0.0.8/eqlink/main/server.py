...
def not_exist_check(p_dict, p_key):
    if p_dict.__contains__(p_key) is False or p_dict[p_key] is None or p_dict[p_key] == '':
        return True
    return False
...
class LinkServer:
    def __init__(self, server_conf, local_storage_path):
        ...
        print(server_conf)
        self.server_conf['HOST'] = '127.0.0.1' if not_exist_check(server_conf, 'HOST') else server_conf['HOST']
        self.server_conf['PORT'] = 7900 if not_exist_check(server_conf, 'PORT') else server_conf['PORT']
        self.server_conf['BACKLOG'] = 7900 if not_exist_check(server_conf, 'BACKLOG') else server_conf['BACKLOG']
        self.server_conf['BUF_SIZE'] = 7900 if not_exist_check(server_conf, 'BUF_SIZE') else server_conf['BUF_SIZE']
