...
async def connect_and_consume_rpc(debug):
    ...
    while connection is None:
        try:
            if debug:
                print_flush(f"Connecting to rabbitmq in connect_and_consume_rpc with c2 {hostname}")
            ...
            if debug:
                print_flush(f"Successfully connected in connect_and_consume_rpc with c2 {hostname}")
    ...
    if debug:
            print_flush("Successfully declared container specific rpc queue in connect_and_consume_rpc with c2 {hostname}")
    await channel.set_qos(prefetch_count=10)
    try:
        if debug:
            print_flush("Starting to consume callbacks in connect_and_consume_rpc with c2 {hostname}")
        await queue.consume(
...
async def connect_and_consume_mythic_rpc(debug):
    ...
    while connection is None:
        try:
            if debug:
                print_flush(f"Connecting to rabbitmq in connect_and_consume_mythic_rpc with c2 {hostname}")
            connection = await aio_pika.connect_robust(
                host=settings.get("host", "127.0.0.1"),
                login=settings.get("username", "mythic_user"),
                password=settings.get("password", "mythic_password"),
            ...
            if debug:
                print_flush(f"Successfully connected in connect_and_consume_mythic_rpc with c2 {hostname}")
        except (ConnectionError, ConnectionRefusedError) as c:
            print_flush("Connection to rabbitmq failed, trying again...")
        except Exception as e:
    ...
    # get a random queue that only the apfell server will use to listen on to catch all heartbeats
    if debug:
        print_flush(f"Declaring container specific mythic rpc queue in connect_and_consume_mythic_rpc with c2 {hostname}")
...
async def mythic_service(debug: bool):
    ...
    if debug:
        print_flush(f"Setting c2 name to {hostname}")
    container_files_path = pathlib.Path(
        os.path.abspath(settings.get("container_files_path", "/Mythic/"))
    )
    if debug:
        print_flush(f"Setting the absolute path to the c2's mythic folder as: {container_files_path}")
